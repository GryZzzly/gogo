...
label aqua_sex:
    ...
    if M_aqua.is_state(S_aqua_mate):
        call expression game.dialog_select("aqua_sex_pre_first") from _call_expression_279

    call expression game.dialog_select("aqua_sex_pre") from _call_expression_280
    if M_aqua.is_state(S_aqua_mate):
        label aqua_sex_replay:
            call expression game.dialog_select("aqua_sex_after_first") from _call_expression_281

    call expression game.dialog_select("aqua_sex_after") from _call_expression_282
...
label aqua_sex_loop:
    ...
    while animcounter < 4:
        if anim_toggle:
            show expression AnimatedImage("aquas", [3,4,5,6,7], M_aqua) as aquas
            pause 5
            call expression game.dialog_select("aqua_hscene_dialog") from _call_expression_283
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            call expression game.dialog_select("aqua_hscene_dialog") from _call_expression_284
        $ animcounter += 1
...
label aqua_sex_cum:
    call expression game.dialog_select("aqua_sex_cum_pre") from _call_expression_285
    if not store._in_replay == None or M_aqua.is_state(S_aqua_mate):
        call expression game.dialog_select("aqua_sex_cum_first") from _call_expression_286
