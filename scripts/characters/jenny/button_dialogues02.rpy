...
label sybian_cum:
    call expression game.dialog_select("sybian_cum_dialogue") from _call_expression_1499
...
label jenny_dialogue_make_deal:
    ...
    call expression game.dialog_select("jenny_dialogue_make_deal_pre{}".format(label_format)) from _call_expression_1500
    ...
    menu:
        "Alright." if player.has_money(500):
            ...
            label jenny_dialogue_make_deal_replay:
                call expression game.dialog_select("jenny_dialogue_make_deal_alright{}".format(label_format)) from _call_expression_1501
            menu sis_titsplay_menu:
                "Continue.":
                    call expression game.dialog_select("jenny_dialogue_make_deal_continue") from _call_expression_1502
                    jump expression game.dialog_select("sis_titsplay_menu")

                "Suck on it." if store._in_replay == None and player.stats.dex() < 5:
                    call expression game.dialog_select("jenny_dialogue_make_deal_suck_stat_fail") from _call_expression_1503

                "Suck on it." if not store._in_replay == None or player.stats.dex() >= 5:
                    call expression game.dialog_select("jenny_dialogue_make_deal_suck_stat_pass") from _call_expression_1504
                    menu sis_titssuck_menu:
                        "Keep going.":
                            call expression game.dialog_select("jenny_dialogue_make_deal_suck_continue") from _call_expression_1505
                            jump expression game.dialog_select("sis_titssuck_menu")
                        "Stop.":

                            call expression game.dialog_select("jenny_dialogue_make_deal_suck_stop") from _call_expression_1506
                            $ renpy.end_replay()
                            $ persistent.cookie_jar["Jenny"]["unlocked"] = True
                            $ persistent.cookie_jar["Jenny"]["gallery"]["04_unlocked"] = True

                "Stop." if store._in_replay == None:
                    call expression game.dialog_select("jenny_dialogue_make_deal_stop") from _call_expression_1507

        "Not yet" if player.has_money(500):
            call expression game.dialog_select("jenny_dialogue_make_deal_not_yet") from _call_expression_1508

        "I don't have enough." if not player.has_money(500):
            call expression game.dialog_select("jenny_dialogue_make_deal_not_enough") from _call_expression_1509
    return
