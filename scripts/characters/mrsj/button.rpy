label mrsj_button_dialogue:
    if player.location == L_erikhouse_entrance:
        ...
        if game.timer.is_dark() and mrsj.over(mrsj_sex_ed):
            call expression game.dialog_select("button_mrsj_sex_ed_intro") from _call_expression_2158
            jump mrsj_button_dialogue_repeat

        elif game.timer.is_dark() and erik.over(erik_gf):
            call expression game.dialog_select("button_mrsj_private_yoga_intro") from _call_expression_2159
            jump mrsj_button_dialogue_repeat
        scene erik_house_upstairs_night_c01
    call expression game.dialog_select("button_mrsj_greetings") from _call_expression_2160
    menu mrsj_button_dialogue_repeat:
        "About {b}Erik{/b}." if erik.started(erik_path_split):
            call screen route_warning
            call expression game.dialog_select("button_mrsj_about_erik") from _call_expression_2161
            menu mrsj_route_split:
                "Sex education.":
                    call expression game.dialog_select("button_mrsj_route_sex_ed") from _call_expression_2162
                ...
                "Get him a girlfriend.":
                    ...
                    call expression game.dialog_select("button_mrsj_route_gf") from _call_expression_2163
                    $ erik_path_split.finish()
                    $ June.add_event(june_intro)
                    jump mrsj_button_dialogue_repeat

        "Sex education." if mrsj.started(mrsj_sex_ed):
            call expression game.dialog_select("button_mrsj_sex_ed_prep") from _call_expression_2164
        ...
        "Girlfriend." if erik.started(erik_gf):
            call expression game.dialog_select("button_mrsj_erik_got_gf") from _call_expression_2165
            $ erik_gf.finish()
            call expression game.dialog_select("erik_house_dialogue") from _call_expression_2166

        "Girlfriend." if erik.in_progress(erik_gf_stolen):
            call expression game.dialog_select("button_mrsj_erik_stole_gf") from _call_expression_2167
            call expression game.dialog_select("erik_house_dialogue") from _call_expression_2168

        "Girlfriend." if June.started(june_intro_2):
            call expression game.dialog_select("button_mrsj_erik_introduce_june") from _call_expression_2169
            $ june_intro_2.finish()
            jump mrsj_button_dialogue_repeat
        ...
        "Breastfeeding." if erik.over(erik_breastfeeding_2) and (not erik.known(erik_sex_ed) and not mrsj.known(mrsj_sex_ed) and not June.known(june_intro)):
            call expression game.dialog_select("button_mrsj_breastfeeding") from _call_expression_2170

        "What did you need me to do?" if mrsj.started(mrsj_yoga_help):
            call expression game.dialog_select("button_mrsj_yoga_help_repeat") from _call_expression_2171
        ...
        "You're so fit!" if not game.timer.is_dark():
            call expression game.dialog_select("button_mrsj_youre_so_fit") from _call_expression_2172
            jump mrsj_button_dialogue_repeat
        ...
        "Invite to poker." if mrsj.known(mrsj_poker_night) and not game.timer.is_night():
            ...
            if game.timer.is_morning() and not game.timer.is_weekend():
                ...
                if mrsj.completed(mrsj_poker_night_2):
                    ...
                    call expression game.dialog_select("button_mrsj_invite_poker") from _call_expression_2173
                $ poker_bot02 = "mrsj"
                $ poker_sayer02 = mrsjo
