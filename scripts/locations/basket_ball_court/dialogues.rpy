...
label basket_ball_court_take_magazines:
    if player.location.is_here(M_dexter):
        if not M_ross.get("take porno fail"):
            call expression game.dialog_select("basketball_court_ross_magazines_intro") from _call_expression_1654
        else:

            call expression game.dialog_select("basketball_court_ross_magazines_retry") from _call_expression_1655

        if player.has_required_dex(3):
            call expression game.dialog_select("basketball_court_ross_magazines_dex_pass") from _call_expression_1656
            call expression "player_ross_magazines_{}_left".format(M_ross.get("magazines remaining")) from _call_expression_1657
            $ M_ross.set("magazine dexter", True)
        else:

            call expression game.dialog_select("basketball_court_ross_magazines_dex_fail") from _call_expression_1658
