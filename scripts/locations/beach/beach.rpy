label beach_dialogue:
    ...
    if M_roxxy.is_state(S_roxxy_spin_bottle) and game.timer == Date("afternoon", "saturday"):
        if player.has_item("goldschwagger"):
            call expression game.dialog_select("beach_roxxy_spin_bottle_goldschwagger") from _call_expression_844
            $ game.timer.tick()
            $ player.remove_item("goldschwagger")
            $ M_roxxy.trigger(T_roxxy_spun_bottle)
        else:
            call expression game.dialog_select("beach_roxxy_spin_bottle_no_goldschwagger") from _call_expression_845
            $ player.go_to(L_map)

    elif M_roxxy.is_state(S_roxxy_spin_bottle) and game.timer == Date("evening", "saturday"):
        call expression game.dialog_select("beach_roxxy_spin_bottle_wrong_time") from _call_expression_846
        $ player.go_to(L_map)
    $ game.main()
...
label beach_island_dialogue:
    ...
    if M_aqua.is_state(S_aqua_treasure_search):
        call expression game.dialog_select("beach_island_aqua_treasure_search") from _call_expression_847
    $ game.main()
...
label beach_water_dialogue:
    ...
    if M_roxxy.is_state(S_roxxy_invite_to_bikini_contest) and game.timer == Date("afternoon", "saturday"):
        call expression game.dialog_select("beach_roxxy_invite_to_bikini_contest") from _call_expression_848
        ...
        call expression game.dialog_select("beach_roxxy_spin_bottle_sex_intro") from _call_expression_849
...
label beach_tower_dialogue:
    ...
    if M_roxxy.is_state(S_roxxy_get_oil) and not player.has_item("massage_oil"):
        call expression game.dialog_select("beach_tower_roxxy_get_oil") from _call_expression_850
    $ game.main()

label beach_cabin_dialogue:
    $ player.go_to(L_beach_cabin)
    if M_roxxy.is_state(S_roxxy_in_cabin):
        call expression game.dialog_select("beach_cabin_roxxy_in_cabin") from _call_expression_851
        $ M_roxxy.trigger(T_roxxy_bikini_failure)
        $ player.go_to(L_beach_water)

    elif M_roxxy.is_state(S_roxxy_get_new_bikini):
        if player.has_item("sara_bikini"):
            call expression game.dialog_select("beach_cabin_roxxy_has_bikini") from _call_expression_852
            $ M_roxxy.trigger(T_roxxy_get_oil)
            $ player.remove_item("sara_bikini")
            $ player.go_to(L_beach_water)
        else:

            call expression game.dialog_select("beach_cabin_roxxy_get_new_bikini") from _call_expression_853
            $ player.go_to(L_beach_water)

    elif M_roxxy.is_state(S_roxxy_get_oil):
        if player.has_item("massage_oil"):
            call expression game.dialog_select("beach_cabin_roxxy_has_oil") from _call_expression_854
            $ M_roxxy.trigger(T_roxxy_contest_over)
            $ game.timer.tick()
            $ player.go_to(L_beach_water)
        else:

            call expression game.dialog_select("beach_cabin_roxxy_get_oil") from _call_expression_855
            $ player.go_to(L_beach_water)

    elif M_roxxy.get("massage"):
        label roxxy_massage_hscene_replay:
            call expression game.dialog_select("beach_cabin_roxxy_massage") from _call_expression_856
        $ renpy.end_replay()
...
label beach_statue:
    if not player.has_item("shovel"):
        call expression game.dialog_select("beach_statue_no_shovel") from _call_expression_857
        $ game.main()

    if M_aqua.is_state(S_aqua_treasure_search):
        call expression game.dialog_select("beach_statue_aqua_treasure_search") from _call_expression_858
    ...
    call expression game.dialog_select("treasure_lock_intro") from _call_expression_859
    call screen treasure_lock

label treasure_open:
    if not player.has_item("treasure_key"):
        call expression game.dialog_select("treasure_open_no_key") from _call_expression_860
    else:
