label church_angelicas_room_dialogue:
    ...
    if M_mia.is_state(S_mia_church_plan) and game.timer.is_weekend() and game.timer.is_morning():
        call expression game.dialog_select("church_angelicas_room_mia_church_plan") from _call_expression_953

    elif M_mia.is_state(S_mia_return_priest_outfit):
        call expression game.dialog_select("church_angelicas_room_mia_return_priest_outfit") from _call_expression_954
        ...
        call expression game.dialog_select("church_angelicas_room_mia_church_night_visit") from _call_expression_955
        $ M_mia.trigger(T_angelica_ritual_deal)

    elif M_mia.is_state(S_mia_church_sacrement) and game.timer.is_dark():
        call expression game.dialog_select("church_angelicas_room_mia_church_sacrement") from _call_expression_956
        $ game.timer.tick()
        $ M_mia.trigger(T_helen_angelica_ritual)
        $ player.go_to(L_map)

    elif M_mia.is_state(S_mia_angelicas_order) and game.timer.is_dark():
        call expression game.dialog_select("church_angelicas_room_mia_angelicas_order") from _call_expression_957
