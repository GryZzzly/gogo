label right_tombstone:
    ...
    if M_aqua.is_state(S_aqua_graveyard_search):
        call expression game.dialog_select("right_tombstone_aqua_graveyard_search") from _call_expression_861
...
label stray_cat:
    ...
    if not M_player.is_set("found cat"):
        ...
        call expression game.dialog_select("stray_cat_first_pre") from _call_expression_862
        if player.has_item("cat_food"):
            call expression game.dialog_select("feed_cat") from _call_expression_863
            $ player.remove_item("cat_food")
            jump expression game.dialog_select("stray_cat_take_home")
        call expression game.dialog_select("stray_cat_first_after") from _call_expression_864

    elif not player.has_item("cat_food"):
        call expression game.dialog_select("stray_cat_no_food") from _call_expression_865

    elif player.has_item("cat_food"):
        call expression game.dialog_select("stray_cat_have_food_pre") from _call_expression_866
        $ player.remove_item("cat_food")
        menu stray_cat_take_home:
            "Take her home.":
                call expression game.dialog_select("stray_cat_have_food_take_her_pre") from _call_expression_867
                call screen cat_name_input
                if cat_name.strip() == "":
                    $ cat_name = "Pussywillow"
                $ cat = Character("[cat_name]", color = "#c87efe")
                call expression game.dialog_select("stray_cat_have_food_take_her_after") from _call_expression_868
                $ M_player.set("pet cat", True)
            "Leave her here.":

                call expression game.dialog_select("stray_cat_have_food_leave_her") from _call_expression_869
