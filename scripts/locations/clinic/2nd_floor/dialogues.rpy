...
label hospital_second_floor_phone_dialogue:
    ...
    if mrsj.started(mrsj_sex_ed) and Roz.completed(roz_intro) and Roz.known(roz_storage) and not Roz.known(roz_trick):
        call expression game.dialog_select("hospital_second_floor_phone_mrsj_sex_ed_started") from _call_expression_1441
        $ Roz.add_event(roz_trick)
        $ M_roz.place(place = L_NULL)
        $ M_roz.force()
    else:

        call expression game.dialog_select("hospital_second_floor_phone_nothing") from _call_expression_1442
    $ game.main()
