...
label hospital_storage_cabinet_dialogue:
    ...
    if mrsj.started(mrsj_sex_ed) and not player.has_item("birth_control_pills"):
        call expression game.dialog_select("hospital_storage_cabinet_mrsj_sex_ed_started") from _call_expression_1240
...
label roz_storage_sex:
    ...
    if M_roz.is_state(S_roz_end):
        call expression game.dialog_select("roz_storage_sex_pre_repeat") from _call_expression_1241
    else:

        call expression game.dialog_select("roz_storage_sex_pre_first") from _call_expression_1242
    call expression game.dialog_select("roz_storage_sex_pre_after") from _call_expression_1243
    $ anim_toggle = True
...
label roz_storage_sex_loop:
    ...
    while animcounter < 4:
        if anim_toggle:
            show expression AnimatedImage("rozs", [1,2,3,4,5,6,7], M_roz) as rozs at right
            pause 5
            call expression game.dialog_select("roz_storage_hscene_dialog") from _call_expression_1244
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            call expression game.dialog_select("roz_storage_hscene_dialog") from _call_expression_1245
        $ animcounter += 1
...
label roz_storage_sex_cum:
    call expression game.dialog_select("roz_storage_sex_cum_pre") from _call_expression_1246
    $ renpy.end_replay()
    $ persistent.cookie_jar["Roz"]["unlocked"] = True
    $ persistent.cookie_jar["Roz"]["gallery"]["01_unlocked"] = True
    scene location_hospital_sex with fade
    if M_roz.is_state(S_roz_end):
        call expression game.dialog_select("roz_storage_sex_cum_after_repeat") from _call_expression_1247
    else:

        call expression game.dialog_select("roz_storage_sex_cum_after_first") from _call_expression_1248
