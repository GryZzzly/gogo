label erik_room_dialogue:
    ...
    if erik.over(erik_crown_card) and not erik.known(erik_orcette):
        ...
        call expression game.dialog_select("erik_bullying") from _call_expression_989
        $ erik.add_event(erik_bullying_2)
        $ M_erik.unforce()

    elif erik.completed(erik_bullying_3) and not erik.known(erik_vr):
        call expression game.dialog_select("eriksroom_erik_bullying_3_completed") from _call_expression_990

    elif erik.over(erik_path_split) and erik.started(erik_sex_ed):
        call expression game.dialog_select("erik_sex_ed") from _call_expression_991
        $ erik_sex_ed.finish()
        $ M_erik.place(place = L_erikhouse_mrsjroom)

    elif June.started(june_intro):
        call expression game.dialog_select("june_intro") from _call_expression_992
        $ june_intro.finish()
        jump erik_talk

    elif erik.started(erik_breastfeeding):
        call expression game.dialog_select("eriksroom_erik_breastfeeding_started") from _call_expression_993
