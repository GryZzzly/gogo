...
label dirt_pile:
    if player.has_item("shovel"):
        call expression game.dialog_select("forest_dirt_pile_have_shovel_intro") from _call_expression_1862
        menu:
            "Dig with a shovel." if player.has_item("shovel"):
                call expression game.dialog_select("forest_dirt_pile_have_shovel_dig") from _call_expression_1863
                call screen forest_worms
            "Leave it alone.":

                call expression game.dialog_select("forest_dirt_pile_have_shovel_leave") from _call_expression_1864
    else:

        call expression game.dialog_select("forest_dirt_pile_no_shovel") from _call_expression_1865
    $ game.main()
...
label mushroom:
    call expression game.dialog_select("forest_mushroom_intro") from _call_expression_1866
    if M_okita.is_state(S_okita_get_ingredients):
        call expression game.dialog_select("forest_mushroom_okita_get_ingredients") from _call_expression_1867

    if M_aqua.is_state(S_aqua_seasucc_mushroom):
        call expression game.dialog_select("forest_mushroom_aqua_seasucc_mushroom") from _call_expression_1868

    call expression game.dialog_select("forest_mushroom_take_mushroom") from _call_expression_1869
...
label altar:
    ...
    call expression game.dialog_select("altar_intro_pre") from _call_expression_1870
    if not game.timer.is_dark():
        call expression game.dialog_select("altar_intro_day") from _call_expression_1871
    else:

        call expression game.dialog_select("altar_intro_night") from _call_expression_1872
        $ game.timer.tick()
        label altar_puzzle:
...
label altar_puzzle_finish:
    call expression game.dialog_select("altar_puzzle_finish_dialogue") from _call_expression_1873
