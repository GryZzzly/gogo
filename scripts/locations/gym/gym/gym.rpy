label training_dialogue:
    $ player.go_to(L_gym)
    if training_count == 0:
        call expression game.dialog_select("gym_training_dialogue_first") from _call_expression_2046
        ...
        call expression game.dialog_select("gym_training_dialogue_second") from _call_expression_2047
        $ training_count = 2
        $ sister.add_event(sis_panty02)

    elif training_count == 2 and training_tier == 3 and sister.over(sis_couch02):
        call expression game.dialog_select("gym_training_dialogue_third") from _call_expression_2048
        $ training_count = 3
        $ sister.add_event(sis_panty03)

    elif training_count == 3 and training_tier == 4 and sister.over(sis_couch03):
        call expression game.dialog_select("gym_training_dialogue_fourth") from _call_expression_2049
        $ training_count = 4
