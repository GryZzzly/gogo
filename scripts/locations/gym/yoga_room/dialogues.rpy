label yoga_room_mrsj_yoga_help_started:
    ...
    if yoga_fail_retry:
        call expression game.dialog_select("yoga_room_mrsj_yoga_help_started_pre_repeat") from _call_expression_1560
    else:

        call expression game.dialog_select("yoga_room_mrsj_yoga_help_started_pre_first") from _call_expression_1561

    label anna_yoga_lesson:
        call expression game.dialog_select("yoga_room_mrsj_yoga_help_started_intro") from _call_expression_1562
    $ yoga_position = ""
    $ yoga_stage = 1
    $ boner_fail = False
    $ game.timer.tick()
    label yoga_room_class:
        call expression game.dialog_select("yoga_room_class_position_pre") from _call_expression_1563
    menu:
        "Happy Baby." if store._in_replay == None or yoga_stage == 2:
            $ yoga_position = "Happy Baby"
            if yoga_stage == 2:
                $ yoga_stage += 1
                call expression game.dialog_select("yoga_room_class_2nd_position_success") from _call_expression_1564
                jump expression game.dialog_select("yoga_room_class")
            else:

                call expression game.dialog_select("yoga_room_class_failure") from _call_expression_1565

        "Plow Position." if store._in_replay == None or yoga_stage == 3:
            $ yoga_position = "Plow Position"
            if yoga_stage == 3:
                $ yoga_stage += 1
                call expression game.dialog_select("yoga_room_class_3rd_position_success") from _call_expression_1566
                call expression game.dialog_select("yoga_room_class_success") from _call_expression_1567
            else:

                call expression game.dialog_select("yoga_room_class_failure") from _call_expression_1568

        "Downward Dog." if store._in_replay == None or yoga_stage == 1:
            $ yoga_position = "Downward Dog"
            if yoga_stage == 1:
                $ yoga_stage += 1
                call expression game.dialog_select("yoga_room_class_1st_position_success") from _call_expression_1569
                jump expression game.dialog_select("yoga_room_class")
            else:

                call expression game.dialog_select("yoga_room_class_failure") from _call_expression_1570

        "End Lesson." if store._in_replay == None and mrsj.completed(mrsj_yoga_help):
            call expression game.dialog_select("yoga_room_class_end_lesson") from _call_expression_1571
    $ game.main()
...
label yoga_room_class_position_pre:
    if yoga_stage == 1:
        call expression game.dialog_select("yoga_room_class_1st_position_pre") from _call_expression_1572

    elif yoga_stage == 2:
        call expression game.dialog_select("yoga_room_class_2nd_position_pre") from _call_expression_1573

    elif yoga_stage == 3:
        $ boner_fail = True
        call expression game.dialog_select("yoga_room_class_3rd_position_pre") from _call_expression_1574
    return
...
label yoga_room_class_failure:
    call expression game.dialog_select("yoga_room_class_failure_dialogue") from _call_expression_1575
    return
...
label yoga_room_class_success:
    call expression game.dialog_select("yoga_room_class_success_pre") from _call_expression_1576
    ...
    call expression game.dialog_select("yoga_room_class_success_after") from _call_expression_1577
    $ renpy.end_replay()
