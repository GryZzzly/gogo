...
label home_front_mom_car_fixed_check_car_finished:
    if M_mom.get("jerk count") == 2:
        call expression game.dialog_select("home_front_mom_car_fixed_check_car_finished_too_much") from _call_expression_529
    else:
        call expression game.dialog_select("home_front_mom_car_fixed_check_car_finished_not_enough") from _call_expression_530
    call expression game.dialog_select("home_front_mom_car_fixed_check_car_finished_after") from _call_expression_531
...
label player_mailbox_dialogue:
    ...
    if erik.completed(erik_orcette) and not player.has_item("orcette") and not erik.known(erik_orcette_2) and orcette_mail_lock:
        call expression game.dialog_select("player_mailbox_erik_orcette_completed_pre") from _call_expression_532
        menu:
            ...
            "Open it.":
                ...
                call expression game.dialog_select("player_mailbox_erik_orcette_completed_open") from _call_expression_533

        $ player.get_item("orcette")
        $ game.mail["player"] = ""
        call expression game.dialog_select("player_mailbox_erik_orcette_completed_after") from _call_expression_534

    elif game.mail["player"] == "m_pizza_pamphlet":
        call expression game.dialog_select("player_mailbox_pizza_pamphlet") from _call_expression_535
        $ L_pizzeria_exterior.unlock()
        $ L_dealership_front.unlock()

    elif game.mail["player"] == "m_newspaper":
        call expression game.dialog_select("player_mailbox_newspaper") from _call_expression_536
    $ player.location.call_screen(False)
...
label bad_guys_driveby:
    $ player.go_to(L_home)
    call expression game.dialog_select("bad_guys_driveby_dialogue") from _call_expression_537
