...
label hallway_dialogue:
    ...
    if not game.timer.is_dark():
        if hallway_count == 0:
            label jenny_intro:
            call expression game.dialog_select("hallway_sis_start") from _call_expression_588
            ...
            call expression game.dialog_select("hallway_mom_sis_boobs_afterthoughts") from _call_expression_589
            $ M_mom.trigger(T_mom_sis_nice_boobs)

        elif sister.started(sis_hallway01) and sister.over(sis_shower_cuddle02):
            call expression game.dialog_select("hallway_sis_hallway_1_started") from _call_expression_590
            ...
            call expression game.dialog_select("hallway_sis_hallway_2_started") from _call_expression_591
            ...
            call expression game.dialog_select("hallway_sis_final_started") from _call_expression_592


        elif sister.over(sis_final) and not sister.known(sis_final2):
            call expression game.dialog_select("hallway_sis_final_over") from _call_expression_593
        ...
        if M_mom.is_state(S_mom_sleepover_offer) and not game.timer.is_night():
            call expression game.dialog_select("hallway_mom_sleepover_offer") from _call_expression_594
            $ M_mom.trigger(T_mom_sleepover_accept)

        elif M_mom.is_state(S_mom_movie_night_two) and game.timer.is_evening():
            call expression game.dialog_select("hallway_mom_movie_night_two") from _call_expression_595
