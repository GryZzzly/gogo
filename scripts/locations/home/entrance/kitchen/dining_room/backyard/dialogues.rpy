...
label mom_pool_dialogue:
    if M_mom.is_state(S_mom_fetch_towel) and not player.has_item("towel"):
        call expression game.dialog_select("backyard_mom_fetch_towel_fail") from _call_expression_1276
    else:

        label mom_replay_finger:
            call expression game.dialog_select("backyard_mom_fetch_towel_success") from _call_expression_1277
...
label mom_finger_loop:
    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            pause 4
            call expression game.dialog_select("debbie_finger_hscene_dialog") from _call_expression_1278
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            call expression game.dialog_select("debbie_finger_hscene_dialog") from _call_expression_1279
        $ animcounter += 1
...
label mom_finger_cum:
    call expression game.dialog_select("mom_finger_cum_pre") from _call_expression_1280

    if not store._in_replay == None or M_mom.is_state(S_mom_fetch_towel):
        call expression game.dialog_select("mom_finger_cum_first_time") from _call_expression_1281
        $ renpy.end_replay()
        ...
        call expression game.dialog_select("mom_finger_cum_repeat") from _call_expression_1282
    hide player
    hide debbie
    hide debbie_robe
    with dissolve
...
label mom_midnight_swim:
    call expression game.dialog_select("backyard_mom_midnight_swim") from _call_expression_1283
