label dining_room_dialogue:
    ...
    if sister.started(sis_breakfast):
        call expression game.dialog_select("dining_room_sis_breakfast_started") from _call_expression_949
...
label dining_room_table_dialogue:
    ...
    if M_mom.is_state(S_mom_diane_dinner) and game.timer.is_evening():
        call expression game.dialog_select("dining_room_mom_diane_dinner") from _call_expression_950
...
label dining_room_table_sis:
    ...
    if not sister.completed(sis_breakfast):
        call expression game.dialog_select("dining_room_sis_breakfast") from _call_expression_951

        $ sis_breakfast.finish()
        $ M_jenny.unforce()
    else:

        call expression game.dialog_select("dining_room_sis_breakfast_done") from _call_expression_952
    hide player
    hide jenny
    with dissolve
