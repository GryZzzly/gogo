...
label mom_movie_night:
    ...
    if M_mom.is_state(S_mom_romance_movie):
        call expression game.dialog_select("mom_movie_night_romance_movie") from _call_expression_200

    elif M_mom.is_state(S_mom_romance_movie_two):
        call expression game.dialog_select("mom_movie_night_romance_movie_two") from _call_expression_201
    else:

        label mom_couch_sex_replay:
            call expression game.dialog_select("mom_movie_night_couch_sex_pre") from _call_expression_202
        menu:
            "Blowjob.":
                call expression game.dialog_select("mom_movie_night_couch_sex_blowjob") from _call_expression_203
                menu mom_movie_night_couch_sex_blowjob_options:
                    "Keep going.":
                        call expression game.dialog_select("mom_movie_night_couch_sex_blowjob_keep_going") from _call_expression_204
                        jump expression game.dialog_select("mom_movie_night_couch_sex_blowjob_options")
                    "Cum.":

                        call expression game.dialog_select("mom_movie_night_couch_sex_blowjob_cum") from _call_expression_205
            "Sex.":

                call expression game.dialog_select("mom_movie_night_couch_sex") from _call_expression_206
                menu mom_movie_night_couch_sex_options:
                    "Keep going.":
                        call expression game.dialog_select("mom_movie_night_couch_sex_keep_going") from _call_expression_207
                        jump expression game.dialog_select("mom_movie_night_couch_sex_options")
                    "Cum.":

                        call expression game.dialog_select("mom_movie_night_couch_sex_cum") from _call_expression_208

        call expression game.dialog_select("mom_movie_night_couch_sex_after") from _call_expression_209
        $ renpy.end_replay()
        ...
        call expression game.dialog_select("mom_movie_night_couch_sex_sleep_together_pre") from _call_expression_210
        menu:
            "Yes":
                call expression game.dialog_select("mom_movie_night_couch_sex_sleep_together_yes") from _call_expression_211
                jump expression game.dialog_select("mom_sleeping")
            "No":

                call expression game.dialog_select("mom_movie_night_couch_sex_sleep_together_no") from _call_expression_212
...
label couch_dialogue:
    call expression game.dialog_select("couch_dialogue_pre") from _call_expression_213
    ...
    menu:
        "Leave.":
            call expression game.dialog_select("couch_dialogue_leave") from _call_expression_214
        "Keep watching.":

            label sis_couch_replay_1:
                call expression game.dialog_select("couch_dialogue_keep_watching_pre") from _call_expression_215
            $ renpy.end_replay()
            $ persistent.cookie_jar["Jenny"]["unlocked"] = True
            $ persistent.cookie_jar["Jenny"]["gallery"]["01_unlocked"] = True
            call expression game.dialog_select("couch_dialogue_keep_watching_after") from _call_expression_216
...
label home_livingroom_tv:
    if M_player.get("masturbated tv"):
        if sister.known(sis_couch02) and M_mom.is_set("jerk available"):
            call expression game.dialog_select("home_livingroom_tv_masturbated_tv_sis_couch02_known") from _call_expression_217
        else:

            call expression game.dialog_select("home_livingroom_tv_masturbated_tv") from _call_expression_218
...
label tv_channel_responses:
    if sister.started(sis_couch03) and game.timer.is_evening():
        label sis_couch_footjob:
            call expression game.dialog_select("tv_channel_sis_couch03_started") from _call_expression_219
        ...
        if tv_pink_channel == 7 and not tv_channel_pink:
            ...
            if game.timer.is_evening():
                if sister.completed(sis_couch02) and M_mom.is_set("jerk available"):
                    call expression game.dialog_select("tv_channel_channel_08_mom_jerk_available") from _call_expression_220
                else:

                    call expression game.dialog_select("tv_channel_channel_08_sis_peeping") from _call_expression_221
                    $ sis_couch02.finish()
            ...
            if game.timer.is_evening():
                if sis_watch_tv_tonight:
                    label sis_couch_final:
                        call expression game.dialog_select("sis_couch_final_pre") from _call_expression_222
                    menu:
                        "Jump on her." if store._in_replay == None and player.stats.dex() < 7:
                            call expression game.dialog_select("sis_couch_final_jump_her_stat_fail") from _call_expression_223
                            $ M_player.set("masturbated tv", True)
                            $ sis_watch_tv_tonight = False
                            jump expression game.dialog_select("home_livingroom_dialogue")

                        "Jump on her." if not store._in_replay == None or player.stats.dex() >= 7:
                            call expression game.dialog_select("sis_couch_final_jump_her_stat_pass") from _call_expression_224
                            jump expression game.dialog_select("sis_couch_sex_loop")
                        "Cum.":

                            call expression game.dialog_select("sis_couch_final_cum") from _call_expression_225
                            $ renpy.end_replay()
                            $ M_player.set("masturbated tv", True)
                            $ sis_watch_tv_tonight = False
                            jump expression game.dialog_select("home_livingroom_dialogue")
                    ...
                    label couch_footjob:
                        call expression game.dialog_select("sis_couch_footjob_pre") from _call_expression_226
                    $ anim_toggle = False
                    call expression game.dialog_select("couch_footjob_loop") from _call_expression_227
                    menu couch_footjob_options:
                        "Keep Going.":
                            call expression game.dialog_select("couch_footjob_loop") from _call_expression_228
                            jump expression game.dialog_select("couch_footjob_options")
                        "Cum.":

                            call expression game.dialog_select("couch_footjob_cum") from _call_expression_229
                            $ renpy.end_replay()
                            $ persistent.cookie_jar["Jenny"]["unlocked"] = True
            ...
            call expression game.dialog_select("sis_pink_pass") from _call_expression_230
            $ tv_channel = 7
    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            if animcounter in [1,3]:
                call expression game.dialog_select("jenny_couch_hscene_dialog") from _call_expression_231
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            if animcounter in [1,3]:
                call expression game.dialog_select("jenny_couch_hscene_dialog") from _call_expression_232
        $ animcounter += 1
...
label sis_couch_sex_cum:
    if anim_toggle:
        call expression game.dialog_select("sis_couch_sex_cum_animation") from _call_expression_233
    else:

        call expression game.dialog_select("sis_couch_sex_cum_manual") from _call_expression_234
    call expression game.dialog_select("sis_couch_sex_cum_after") from _call_expression_235
    $ renpy.end_replay()
    $ persistent.cookie_jar["Jenny"]["unlocked"] = True
