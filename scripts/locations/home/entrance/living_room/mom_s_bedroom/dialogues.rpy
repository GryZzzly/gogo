label moms_bedroom_night:
    scene debbie_peek_sequence_1_night
    player_name "( {b}[deb_name]'s{/b} sleeping. )"
    player_name "( I shouldn't make any noise. )"
    return

label mom_spy:
    show debbie_peek_sequence 2_3 zorder 3
    player_name "WHA-!!"
    player_name "{b}[deb_name]{/b}?!?"
    player_name "( ...She's naked on her bed... )"
    player_name "( ...And rubbing herself...moaning. )"
    deb "Oh... Yes..."
    deb "Just like that, sweetie..."
    deb "Mmm..."
    deb "Yes... {b}[firstname]{/b}..."
    deb "Don't stop!"
    player_name "( !!! )"
    deb "Oh god!"
    deb "I'm getting close, sweetie!"
    deb "Fuck me harder! I need to cum so bad!"
    deb "Ahh!"

    deb "YES!!!" with hpunch
    $ player.go_to(L_home_livingroom)
    scene home_livingroom_c
    show player 107 at left
    with fade
    player_name "( This is... I'm feeling... )"
    player_name "( ... This is so hot! )"
    show jenny 10 at right with dissolve
    jen "..."
    show jenny 9 at right
    jen "What are you doing?!"
    show player 23 at left with hpunch
    show jenny 10 at right
    player_name "( !!! )"
    show player 29 at left
    player_name "Err, nothing! I was just-"
    show player 11 at left
    show jenny 9 at right
    jen "Are you spying on {b}[deb_name]{/b}?"
    show player 5 at left
    show jenny 10 at right
    jen "What is she changing or something?"
    show player 10 at left
    player_name "... No."
    show player 5 at left
    show jenny 12 at right
    jen "Well, what then?"
    show jenny 16 at left
    show player 22f at right
    with dissolve
    jen "Out of my way!"
    jen "..."
    show player 5f at right
    jen "Whoa!! She's really going at it in there!"
    show jenny 17f at left
    jen "... And saying your name!"
    show player 80f at right with dissolve
    jen "What the hell?!"
    show player 81f at right with hpunch
    player_name "( !!! )"
    show jenny 12f at left
    show player 78f at right
    jen "Good lord! Are you smuggling a summer sausage in there or what?"
    show player 80f at right
    jen "I hope you aren't spying on me every time I'm feeling naughty..."
    show player 82f at right
    jen "... because that would be a real wimpy move, {b}[firstname]{/b}!"
    jen "What do you have to say for yourself?"
    show player 83f at right
    show jenny 11f at left
    player_name "Please, don't tell {b}[deb_name]{/b} I was watching her..."
    show player 82f at right
    show jenny 19f at left
    jen "Heh, Well... Maybe I will, maybe I won't..."
    show player 83f at right
    show jenny 18f at left
    player_name "What do you mean?"
    show jenny 19f at left
    show player 80f at right
    jen "You have to agree to do what I say, when I say it."
    show player 83f at right
    show jenny 18f at left
    player_name "Uhh, like what?"
    show player 82f at right
    show jenny 19f at left
    jen "I dunno yet. You'll just have to wait and see..."
    show jenny 18f at left
    player_name "..."
    show jenny 12f at left
    jen "Sounds like she's getting close in there... Enjoy the rest of the show, Perv!"
    hide player
    hide jenny
    with dissolve
    $ renpy.end_replay()
    return

label moms_bedroom_mom_panties_masturbation_again:
    scene location_home_debbiebedroom_day_blur
    show player 1 with dissolve
    player_name "( Alright, I'm in! )"
    player_name "( Now I just need to {b}grab a pair of her panties{/b} before getting into her bed. )"
    player_name "( Oh, sweet release... Here I come! )"
    hide player with dissolve
    return

label moms_bedroom_mom_sleepover_makeup:
    scene debbie_bedroom
    show player 264 at left with dissolve
    player_name "{b}*Yawn*{/b}"
    show player 266 with dissolve
    player_name "I can't believe I slept in {b}[deb_name]{/b}'s bed last night..."
    show player 267
    pause
    show player 268
    player_name "I'd better go before she notices my morning wood."
    show player 267
    deb "{b}*Yawn*{/b}"
    player_name "( !!! )"
    show player 261 with dissolve
    pause
    show player 8f with dissolve
    pause
    show debbie 2 at right with dissolve
    show player 22 with dissolve
    deb "I slept like a log!"
    show debbie 1
    show player 21
    player_name "Y-yeah, me too..."
    show xtra 21 at left
    show player 5
    show debbie 3
    deb "That was the best sleep I've had in a long while!"
    show debbie 2
    deb "It felt good having someone here with me again."
    deb "I hadn't realized how much I miss that feeling."
    show debbie 1
    show player 21
    player_name "Yeah, it felt nice."
    show player 5
    show debbie 3
    deb "Heh, you're so cute!"
    show debbie 2
    deb "You'd best get upstairs before {b}[jen_name]{/b} gets up."
    show debbie 1
    show player 21
    player_name "Yeah, I was just leaving."
    show player 5
    show debbie 2
    deb "Just remember to be quiet, sweetie."
    deb "We don't want her finding you're getting special treatment!"
    show debbie 3
    deb "Haha."
    show debbie 1
    show player 21
    player_name "I'll be careful."
    player_name "Feel free to come sleep here again, if you need to."
    show player 13
    show debbie 2
    deb "See you soon, sweetie."
    deb "... And remember. You're always welcome to sleep here."
    hide xtra
    hide player
    hide debbie
    with dissolve
    return

label moms_bedroom_mom_dinner_outfit:
    scene debbie_bedroom with None
    show debbie 144 at left
    show player 11f at right
    with dissolve
    deb "So, what do you think?"
    show debbie 145
    show player 14f
    player_name "You look great, {b}[deb_name]{/b}!"
    show debbie 146
    show player 13f
    deb "Really? How about from the back?"
    show debbie 147 with fastdissolve
    show player 212f
    pause
    show player 21f
    player_name "Even better!"
    show player 1f
    deb "You're sure?"
    deb "It's pretty tight..."
    show player 14f
    player_name "No way! Your butt looks perfect, {b}[deb_name]{/b}!"
    show player 212f
    pause
    show player 11f
    deb "Oh, So you're staring at my butt, huh?"
    show player 21f
    player_name "... Maybe... A little."
    show player 1f
    show debbie 148 with fastdissolve
    deb "Heh, well at least you're honest about it!"
    deb "I'm glad you like it..."
    show debbie 149
    show player 29f
    player_name "Heh, y-yeah..."
    hide debbie
    hide player
    with dissolve
    scene home_livingroom_b
    show player 35
    with dissolve
    player_name "I'd better not forget that {b}fish{/b} for dinner tonight. What kind did she ask for again?"
    show player 34
    return

label moms_bedroom_mom_dinner_outfit_ask:
    scene debbie_bedroom
    show debbie 159 at left
    with fade
    pause
    show debbie 160 with fastdissolve
    pause
    show debbie 161b
    show player 14f at right with fastdissolve
    player_name "Hey {b}[deb_name]{/b}, what was-"
    show player 11f
    player_name "( !!! )" with hpunch
    show debbie 161
    deb "Sweetie! I'm still changing!"
    show player 21f
    show debbie 152 with fastdissolve
    player_name "Oh, I... I'm sorry!"
    show debbie 153
    show player 13f
    deb "Heh, it's okay."
    deb "It's nothing you haven't seen before."
    show debbie 152
    show player 14f
    player_name "Uhh... So, what was the name of that {b}fish{/b} again? The one {b}Diane{/b} likes so much?"
    show debbie 153
    show player 1f
    deb "{b}Sea Trout{/b}, sweetie."
    show debbie 152
    show player 14f
    player_name "Got it! I'll be sure to pick one up!"
    show debbie 154
    show player 11f
    deb "Wait!"
    show debbie 155
    show player 10f
    player_name "Hmm?"
    show debbie 156 with fastdissolve
    show player 11f
    deb "You like my new lingerie?"
    show debbie 157
    show player 21f
    player_name "Uhhh..."
    player_name "It's really sexy, {b}[deb_name]{/b}."
    show debbie 158
    show player 13f
    deb "Hehe, you're just the sweetest!"
    deb "Come here!"
    show debbie 162 at Position(xpos=102)
    hide player
    with fastdissolve
    pause
    show debbie 163
    player_name "( !!! )" with hpunch
    show debbie 155 at left
    show player 21f at right
    with fastdissolve
    player_name "Thanks, {b}[deb_name]{/b}!"
    player_name "I'd better get going."
    show debbie 154
    show player 13f
    deb "Yeah. I'll see you tonight, sweetie."
    scene home_livingroom_b
    show player 21
    with fade
    player_name "( {b}Wow{/b}! )"
    player_name "( {b}[deb_name]{/b} is so hot! )"
    hide player with dissolve
    return

label moms_bedroom_mom_midnight_swim_after:
    scene home_livingroom_night_b
    show player 25f with dissolve
    player_name "I should leave her alone..."
    show player 24f
    player_name "She seemed pretty upset..."
    hide player with dissolve
    return

label moms_bedroom_mom_is_set_lotion:
    scene debbie_peek_sequence_1
    player_name "( {b}[deb_name]'s{/b} not in her room. )"
    player_name "( She's probably in the kitchen or something... )"
    return


label mom_drawer:
    python:
        for image in renpy.get_showing_tags():
            renpy.hide(image)
    call screen moms_drawer

label mom_drawer_panties:
    call expression game.dialog_select("mom_drawer_panties_dialogue") from _call_expression_24
    $ M_mom.trigger(T_mom_steal_panties)
    $ M_mom.set("panties taken", True)

    $ game.main()

label mom_drawer_panties_dialogue:
    scene debbie_bedroom
    show player 490
    player_name "( Wow, they're so soft. )"
    show player 489
    pause
    show player 491
    pause
    show player 490
    player_name "( ... Hmm, smells like fresh linens. )"
    show player 489
    player_name "( ... )"
    show player 490
    player_name "( I bet her bed smells like her... )"
    player_name "( Maybe, I could lay on it for just a little bit? )"
    player_name "( She won't ever find out. )"
    hide player
    with dissolve
    return

label mom_bed:
    if M_mom.is_set("panties taken"):
        scene location_debbiebed01 with dissolve
        if M_mom.is_state(S_mom_panties_masturbation):
            call expression game.dialog_select("mom_bed_panties_masturbation_first") from _call_expression_25
            $ M_mom.trigger(T_mom_caught_masturbating)
            $ player.go_to(L_home_livingroom)

        elif M_mom.is_set("sex available"):
            call expression game.dialog_select("mom_bed_panties_masturbation_repeat") from _call_expression_26
        else:

            label replay_mom_panties:
                call expression game.dialog_select("mom_bed_panties_masturbation_pre") from _call_expression_27
            if store._in_replay == None and not M_mom.is_state(S_mom_panties_masturbation_again):
                jump expression game.dialog_select("mom_jerk_repeat")
            deb "You promised you wouldn't do this in my room!"
            if not store._in_replay == None:
                jump expression game.dialog_select("replay_mom_panties_2")
            menu:
                "I can't help it!":
                    label replay_mom_panties_2:
                        call expression game.dialog_select("mom_bed_panties_masturbation_cant_help_it") from _call_expression_28
                    if not store._in_replay == None:
                        jump expression game.dialog_select("mom_jerk_repeat")
                    menu:
                        "I'm trying.":
                            call expression game.dialog_select("mom_bed_panties_masturbation_im_trying") from _call_expression_29
                        "I like you.":

                            label mom_jerk_repeat:
                                $ M_mom.set("sex speed", .15)
                                $ M_mom.set("no panties", False)
                            if not store._in_replay == None or L_home_basement.is_here(M_mom):
                                if store._in_replay == None or not store._in_replay == None and renpy.random.random() > 0.5:
                                    $ M_mom.set("no panties", True)
                            call expression game.dialog_select("mom_bed_panties_masturbation_i_like_you") from _call_expression_30
                            $ renpy.end_replay()
                            $ persistent.cookie_jar["Debbie"]["unlocked"] = True
                            $ persistent.cookie_jar["Debbie"]["gallery"]["02_unlocked"] = True
                            $ M_mom.trigger(T_mom_caught_masturbating)
                        "Not really.":

                            call expression game.dialog_select("mom_bed_panties_masturbation_not_really") from _call_expression_31
                "Sorry.":

                    call expression game.dialog_select("mom_bed_panties_masturbation_sorry") from _call_expression_32
        $ M_mom.set("panties taken", False)

        $ game.timer.tick()
        hide debbie
        hide player
        with dissolve

    elif M_mom.is_set("sleep together") and game.timer.is_dark():
        label mom_night_sex_replay:
            if not store._in_replay == None:
                $ M_mom.set("sex available", True)
        scene debbie_bedroom_closeup_sex
        if M_mom.is_set("sex available"):
            call expression game.dialog_select("mom_bed_sleep_together_pre_sex_available") from _call_expression_33
        else:

            call expression game.dialog_select("mom_bed_sleep_together_pre") from _call_expression_34
        $ game.timer.tick()
        menu mom_sleep_options:
            "Cuddle":
                if M_mom.is_set("sex available"):
                    call expression game.dialog_select("mom_bed_sleep_together_cuddle_sex_available") from _call_expression_35
                else:

                    call expression game.dialog_select("mom_bed_sleep_together_cuddle") from _call_expression_36
                menu mom_bed_sleep_together_cuddle_options:
                    "Keep going":
                        call expression game.dialog_select("mom_bed_sleep_together_cuddle_keep_going") from _call_expression_37
                        jump expression game.dialog_select("mom_bed_sleep_together_cuddle_options")
                    "Stop.":

                        call expression game.dialog_select("mom_bed_sleep_together_stop") from _call_expression_38
                        jump expression game.dialog_select("mom_sleep_options")
            "Ask for a kiss":

                if M_mom.is_set("sex available"):
                    call expression game.dialog_select("mom_bed_sleep_together_kiss_sex_available") from _call_expression_39
                else:

                    call expression game.dialog_select("mom_bed_sleep_together_kiss") from _call_expression_40
                menu mom_bed_sleep_together_kiss_options:
                    "Keep going":
                        call expression game.dialog_select("mom_bed_sleep_together_kiss_keep_going") from _call_expression_41
                        jump expression game.dialog_select("mom_bed_sleep_together_kiss_options")
                    "Stop.":

                        call expression game.dialog_select("mom_bed_sleep_together_stop") from _call_expression_42
                        jump expression game.dialog_select("mom_sleep_options")
            "Breastfeed":

                if M_mom.is_set("sex available"):
                    call expression game.dialog_select("mom_bed_sleep_together_breastfeed_sex_available") from _call_expression_43
                else:

                    call expression game.dialog_select("mom_bed_sleep_together_breastfeed") from _call_expression_44
                menu mom_bed_sleep_together_breastfeed_options:
                    "Keep going":
                        call expression game.dialog_select("mom_bed_sleep_together_breastfeed_keep_going") from _call_expression_45
                        jump expression game.dialog_select("mom_bed_sleep_together_breastfeed_options")
                    "Stop.":

                        call expression game.dialog_select("mom_bed_sleep_together_stop") from _call_expression_46
                        jump expression game.dialog_select("mom_sleep_options")
            "Rub":

                if M_mom.is_set("sex available"):
                    call expression game.dialog_select("mom_bed_sleep_together_rub_sex_available") from _call_expression_47
                else:

                    call expression game.dialog_select("mom_bed_sleep_together_rub") from _call_expression_48
                menu mom_bed_sleep_together_rub_options:
                    "Keep going":
                        call expression game.dialog_select("mom_bed_sleep_together_rub_keep_going") from _call_expression_49
                        jump expression game.dialog_select("mom_bed_sleep_together_rub_options")
                    "Stop.":

                        call expression game.dialog_select("mom_bed_sleep_together_stop") from _call_expression_50
                        jump expression game.dialog_select("mom_sleep_options")

            "Fuck" if M_mom.is_set("sex available"):
                $ anim_toggle = False
                $ xray = False
                call expression game.dialog_select("mom_bed_sleep_together_fuck") from _call_expression_51
                menu mom_bed_sleep_together_fuck_options:
                    "Keep going":
                        call expression game.dialog_select("mom_bed_sleep_together_fuck_keep_going") from _call_expression_52
                        jump expression game.dialog_select("mom_bed_sleep_together_fuck_options")
                    "Cum":

                        call expression game.dialog_select("mom_bed_sleep_together_fuck_cum") from _call_expression_53
                        $ renpy.end_replay()
                        $ persistent.cookie_jar["Debbie"]["unlocked"] = True
                        $ persistent.cookie_jar["Debbie"]["gallery"]["06_unlocked"] = True
                        menu:
                            "Stay.":
                                call expression game.dialog_select("mom_bed_sleep_together_fuck_cum_stay") from _call_expression_54
                                jump expression game.dialog_select("mom_sleeping")
                            "Leave.":

                                call expression game.dialog_select("mom_bed_sleep_together_fuck_cum_leave") from _call_expression_55
                                $ player.go_to(L_home_livingroom)

            "Sleep with {b}[deb_name]{/b}." if store._in_replay == None:
                call expression game.dialog_select("mom_bed_sleep_together_sleep") from _call_expression_56
                jump expression game.dialog_select("mom_sleeping")

    elif game.timer.is_dark():
        call expression game.dialog_select("mom_bed_night") from _call_expression_57
    else:

        call expression game.dialog_select("mom_bed_day") from _call_expression_58
    $ game.main()

label mom_bed_panties_masturbation_first:
    player_name "( These panties are really sexy... )"
    show location_debbiebed 2_3
    pause 5
    scene location_debbiebed04
    deb "( !!! )" with hpunch
    scene location_debbiebed05
    deb "Oh my..."
    player_name "{b}[deb_name]{/b}?!"
    scene debbie_bedroom_closeup
    show debbie 16 at right
    show player 152 at left
    with dissolve
    deb "I'm sorry, I didn't mean to-"
    show debbie 17
    show player 150
    deb "... I mean, I was expecting-"
    deb "I ehh..."
    show debbie 16
    deb "I should leave!"
    show debbie 15
    show player 149
    player_name "But-"
    show debbie 16
    show player 150
    deb "No, it's fine... Just..."
    show debbie 17
    show player 154
    deb "... Excuse me!"
    player_name "..."
    scene home_livingroom_b
    show debbie 16
    with dissolve
    deb "( I can't believe what I just saw! )"
    deb "( Was he really just doing that?! )"
    show debbie 20
    deb "( ... In MY room?! )"
    show debbie 17
    deb "( Why would he be doing that in my room?! )"
    show debbie 16
    deb "( Surely he can't be attracted to me, can he?! )"
    show debbie 19
    deb "..."
    show debbie 18
    deb "( It's probably normal... )"
    deb "( He's a young man and his hormones are making him do crazy things! )"
    deb "( That's gotta be it! He's probably just confused after everything that's happened recently... )"
    show debbie 17
    deb "( ... Should I talk to him about it? )"
    show debbie 16
    deb "( I suppose, I should at least ask him to do that kind of thing in his own room... And not with {b}my underwear{/b}! )"
    scene debbie_bedroom_closeup
    show player 151 at left
    with dissolve
    player_name "( Oh, crap. This is bad! )"
    player_name "( I'm {b}SO{/b} screwed... )"
    player_name "( Why did I even DO this! )"
    show player 153
    player_name "...Ugh..."
    player_name "( I'm so stupid... )"
    show player 151
    player_name "( ... Things were doing so well between us and now this is going to ruin everything! )"
    scene home_livingroom_b
    show debbie 13 at right with dissolve
    show player 5 at left with dissolve
    deb "Hey, sweetie!"
    show debbie 14
    show player 21
    player_name "..."
    show debbie 13
    show player 24
    deb "Listen, I just want you to know I'm not mad..."
    deb "Masturbation is perfectly normal..."
    deb "I know you have been going through a lot recently; Losing your {b}Dad{/b} and moving in here with us..."
    show player 5
    show debbie 14
    player_name "..."
    show debbie 13
    deb "You've got to be stressed and more than a little confused..."
    deb "... But you shouldn't be doing that in my room! ... And certainly not with my lingerie!"
    show debbie 14
    show player 24
    player_name "I know, I'm sorry. I don't know what came over me!"
    show debbie 13
    show player 5
    deb "It's okay, sweetie..."
    show player 11
    deb "We just need to find you a nice Girlfriend or something..."
    deb "... {b}Maybe give internet porn{/b} a try until then?"
    show debbie 14
    show player 21
    player_name "{b}[deb_name]{/b}!! Please stop, this is all really embarrassing!"
    show debbie 13
    show player 13
    deb "... Sorry. I'm just trying to help."
    deb "Don't be so embarrassed. Really, it's not a big deal... Everyone masturbates. It's perfectly normal!"
    deb "I still think you're a great kid! You're just having a tough time is all..."
    show debbie 14
    show player 21
    player_name "I'm not a kid..."
    show debbie 13
    show player 13
    deb "You're right, I sorry. You aren't a kid. Just promise not to do that in my room anymore."
    show player 21
    show debbie 14
    player_name "I promise..."
    return

label mom_bed_panties_masturbation_repeat:
    player_name "( These feel so soft against my skin... )"
    show location_debbiebed 2_3
    pause 5
    scene location_debbiebed04
    deb "!!!" with hpunch
    scene location_debbiebed05
    deb "Again!"
    player_name "{b}[deb_name]{/b}!!"
    scene debbie_bedroom_closeup
    show player 150 zorder 1 at left
    show debbie 16 at right with dissolve
    deb "Sweetie!"
    show debbie 25b at Position(xpos = 810, ypos = 768) with dissolve
    deb "You're going to ruin all my nice panties!"
    show debbie 26 zorder 0 at Position(xpos = 610) with dissolve
    show player 151
    player_name "Sorry, I just... I can't seem to stop."
    show player 150
    show debbie 25
    deb "It's alright. You're a man and you have urges..."
    deb "Want me to give you another little show to help speed things up?"
    show debbie 26
    show player 154
    player_name "( !!! )"
    show player 149
    player_name "Really?"
    player_name "Yeah!"
    show player 150 zorder 0
    show debbie 25b zorder 1 at Position(xpos = 810) with dissolve
    deb "Alright, sweetie."
    show debbie 19 at right with dissolve
    pause
    show debbie 21 with dissolve
    pause
    show debbie 22b with dissolve
    pause
    show debbie 23d with dissolve
    pause
    show debbie 23e
    deb "How do I look?"
    show debbie 23d
    show player 149
    player_name "Beautiful..."
    show player 150
    show debbie 24b
    show player 155_156_156b_156 with dissolve
    pause
    pause
    player_name "Oh, {b}[deb_name]{/b}! You're so sexy!"
    show player 157_158_159 with flash
    show debbie 24c
    deb "( !!! ){p=1.5}{nw}"
    show player 161 with dissolve
    show debbie 24b
    deb "That almost made it all the way over here!"
    show debbie 23d
    show player 160
    player_name "You're the best {b}[deb_name]{/b}..."
    show player 161
    show debbie 23e
    deb "I'd better be for letting you do this!"
    deb "Now, come over here and give me a hug."
    show debbie 23d
    hide player
    show debbie 28
    with dissolve
    deb "Love you."
    show debbie 29
    player_name "Love you, too."
    return

label mom_bed_panties_masturbation_pre:
    scene location_debbiebed01
    player_name "( These feel so soft against my skin... )"
    show location_debbiebed 2_3
    player_name "( Oh, this is just what I needed... )"
    pause 5
    scene location_debbiebed04
    deb "( !!! )" with hpunch
    scene location_debbiebed05
    deb "Oh my..."
    player_name "{b}[deb_name]{/b}!!"
    scene debbie_bedroom_closeup
    show player 152 at left with dissolve
    show debbie 16 at right with dissolve
    deb "{b}Again{/b}??!"
    show debbie 20
    pause
    deb "..."
    show player 150 zorder 1
    show debbie 25b zorder 0 at Position(xpos=0.675, ypos= 1.0) with dissolve
    pause 1
    show debbie 25 at Position(xpos =0.58, ypos = 768) with dissolve
    deb "Sweetie, we talked about this."
    return

label mom_bed_panties_masturbation_sorry:
    show debbie 26
    show player 151
    player_name "I'm sorry, {b}[deb_name]{/b}..."
    player_name "I don't know what came over me!"
    player_name "...It won't happen again, I promise!"
    show debbie 27
    show player 152
    deb "Oh, sweetie..."
    show debbie 25
    deb "...Just please, try and do it elsewhere from now on."
    deb "Can you do that for me?"
    show debbie 26
    show player 151
    player_name "Yes, Ma'am..."
    return

label mom_bed_panties_masturbation_cant_help_it:
    show debbie 26
    show player 151
    player_name "I'm sorry, {b}[deb_name]{/b}... It just {b}feels so good{/b}..."
    show debbie 27
    show player 152
    deb "I know, sweetie... But it's not right you doing it in my bed... And with my clothes!"
    show debbie 25
    deb "Why can't you find another outlet to release your... {b}Energy{/b} with?"
    deb "What about the girls at your school, don't you like any of them?"
    return

label mom_bed_panties_masturbation_im_trying:
    show debbie 26
    show player 151
    player_name "I'm trying, {b}[deb_name]{/b}..."
    player_name "...It's not easy to meet girls sometimes..."
    show player 152
    deb "..."
    show debbie 25
    deb "Have you tried asking them on dates?"
    show debbie 26
    show player 149
    player_name "What?"
    player_name "I can't just... do it like that..."
    show debbie 25
    show player 152
    deb "Well, you have to make an effort, sweetie!"
    show debbie 26
    show player 151
    player_name "{b}*Sigh*{/b} I will, {b}[deb_name]{/b}."
    show debbie 25
    show player 152
    deb "As long as you're trying..."
    show debbie 25b zorder 0 at Position(xpos=0.675, ypos= 1.0) with dissolve
    deb "Come here..."
    hide player
    show debbie 28
    with dissolve
    player_name "..."
    deb "We're going to get through this..."
    show debbie 29
    player_name "Y-yeah..."

label mom_bed_panties_masturbation_not_really:
    show debbie 26
    show player 151
    player_name "I dunno... Not really..."
    player_name "...I just haven't found one that I like, I guess."
    deb "..."
    show debbie 25
    show player 152
    deb "How about that neighbor girl?"
    show debbie 27
    show player 150
    deb "What's her name again?"
    show debbie 26
    show player 151
    player_name "{b}Mia{/b}..."
    show debbie 25
    show player 152
    deb "Yeah! What about {b}Mia{/b}? She's cute!"
    show debbie 26
    show player 151
    player_name "Yeah, I guess..."
    show debbie 25
    show player 152
    deb "Well, you have to make an effort, sweetie!"
    show debbie 26
    show player 151
    player_name "I'll try, {b}[deb_name]{/b}."
    show debbie 25
    show player 152
    deb "As long as you're trying..."
    show debbie 25b zorder 0 at Position(xpos=0.675, ypos= 1.0) with dissolve
    deb "Come here..."
    hide player
    show debbie 28
    with dissolve
    player_name "..."
    deb "We're going to get through this..."
    show debbie 29
    player_name "Y-yeah..."

label mom_bed_panties_masturbation_i_like_you:
    show debbie 30
    show player 149
    if M_mom.is_state(S_mom_panties_masturbation_again):
        player_name "I like you..."
    else:
        player_name "But I can't get you out of my head..."
    show player 152
    deb "..."
    if M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 25
        show player 150
        deb "You don't mean that... You're just confused."
        show debbie 30
        show player 149
        player_name "No, I really do mean it! {b}[deb_name]{/b}, you're all I think about."
        show player 151
        player_name "The girls my age just can't hold a candle to you!"
        show player 152
        deb "..."
        show debbie 25
        deb "I..."
        show debbie 26
        deb "That's really sweet of you to say, {b}[firstname]{/b} but we can't do this!"
        show player 150
        show debbie 25
        deb "{b}*Sigh*{/b} This is all my fault..."
        deb "... All that kissing..."
        show debbie 27
        deb "Oh god and the thing in the car!"
        show debbie 27
        deb "..."
        deb "I'm a terrible person..."
        show debbie 26
        show player 149
        player_name "It's not your fault, {b}[deb_name]{/b}..."
        show debbie 30
        show player 151
        player_name "It's me! I just don't want anyone else!"
        player_name "... Maybe there's something wrong with me..."
        show debbie 25b zorder 0 at Position(xpos=0.675, ypos= 1.0) with dissolve
        pause.4
        show debbie 15 at Position(xpos=0.8, ypos=1.0) with dissolve
        show player 150
        player_name "..."
        show debbie 17
        deb "{b}*Sigh*{/b}"
        show debbie 18
        deb "Sweetie, there's nothing wrong with you."
        deb "You're just confused... That's all."
    if not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 17 at Position(xpos=0.8, ypos=1.0) with dissolve
    deb "... Maybe you just need to get this out of your system?"
    show debbie 15
    if M_mom.is_state(S_mom_panties_masturbation_again):
        show player 149
        player_name "What do you mean?"
        show debbie 18
        deb "I'm going to do this but {b}only this one time, alright{/b}?"
        show debbie 17
        deb "And you have to promise me that you'll try harder to talk with girls at school!"
        show debbie 19
        show player 149
        player_name "What are you gonna do?"
    show player 154
    show debbie 21
    window hide
    pause
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 22b
    else:
        show debbie 22
    window hide
    pause
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 23b with hpunch
    else:
        show debbie 23 with hpunch
    window hide
    pause
    player_name "( !!! )"
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 23e
    else:
        show debbie 31
    deb "Go ahead, {b}[firstname]{/b}. Finish what you started..."
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 23b
    else:
        show debbie 23
    show player 149
    player_name "...You mean it's okay?!!"
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 23e
    else:
        show debbie 31
    show player 150
    if M_mom.is_state(S_mom_panties_masturbation_again):
        deb "It's okay, {b}just this once{/b}!"
    else:
        deb "Just {b}do it{/b}. It's okay..."
        show debbie 23b
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show player 149
        player_name "{b}[deb_name]{/b}..."
        player_name "...You're not wearing panties?"
        show player 150
        show debbie 23c
        pause
        show debbie 23e
        deb "What's that?"
        deb "Oh! My panties?"
        deb "I guess I just...forgot!"
        show debbie 23d
        player_name "..."
        show debbie 23e
        deb "Does it...help?"
        show debbie 23d
        show player 149
        player_name "I like it."
    else:

        show debbie 23
        show player 155 with dissolve
        player_name "O-okay..."
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show player 155_156_156b_156
    else:
        show player medmo 155_156_156b
    pause
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 24d
    else:
        show debbie 24
    player_name "Oh, {b}[deb_name]{/b}..."
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 24b
    else:
        show debbie 24f
    deb "Oh my..."
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 24d
    else:
        show debbie 24
    pause
    player_name "{b}[deb_name]{/b}!!! I'm gonna-"
    deb "..."
    show player 157 with flash
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 24c
    else:
        show debbie 24e
    player_name "HNNGGG!!"
    pause .4
    show player 158
    pause .4
    show player 159
    pause 1
    show player 157
    pause .4
    show player 158
    pause .4
    show player 159
    pause 1
    show player 157
    pause .4
    show player 158
    pause .4
    show player 159
    pause .2
    show player 160 with dissolve
    player_name "huuuh, huuuh, huuuuh..."
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 24b
    else:
        show debbie 24f
    deb "Wow, that was a lot!"
    player_name "..."
    show player 161
    if M_mom.is_set("no panties") and not M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 22b
    else:
        show debbie 22
    window hide
    pause
    show debbie 21
    window hide
    pause
    if M_mom.is_state(S_mom_panties_masturbation_again):
        show debbie 16
        deb "You have to make an effort to meet other girls now... {b}Promise{/b}?"
        show debbie 15
        show player 160
        player_name "Yeah..."
        show debbie 17
        show player 161
        deb "This was just a {b}one time thing{/b}... To get it out of your system..."
    show debbie 17
    deb "Come here..."
    hide player
    show debbie 28 with dissolve
    pause
    show debbie 29
    player_name "..."
    show debbie 28
    deb "We're going to get through this..."
    show debbie 29
    player_name "Y-yeah..."
    return

label mom_bed_sleep_together_pre_sex_available:
    show player 109 at right
    show debbie 83 at left
    with dissolve
    deb "..."
    show debbie 81
    deb "{b}[firstname]{/b}?"
    deb "Is everything okay?"
    show player 111
    player_name "Hey, {b}[deb_name]{/b}."
    show player 108
    player_name "I'm just having trouble sleeping..."
    player_name "All I can think about is being inside you."
    show player 109
    show debbie 81
    deb "It's okay."
    deb "You have too much energy that needs... releasing."
    deb "Come inside, I know exactly what to do."
    show debbie 82
    show player 108
    player_name "You sure?"
    show debbie 81
    show player 110
    deb "Oh, yes... I need it, too."
    scene debbie_cuddle
    show debbies 77
    with fade

    player_name "Thanks, {b}[deb_name]{/b}."
    show debbies_bed_overlay zorder 3 at Position(xpos=438)
    show debbies 79 zorder 2 at Position(xpos=365)
    with dissolve
    player_name "I couldn't sleep... I was so horny thinking about you..."
    player_name "...And I wanted to be in you again."
    show debbies 80
    deb "It's perfectly normal, sweetie."
    return

label mom_bed_sleep_together_pre:
    show player 109 at right
    show debbie 83 at left
    with dissolve
    deb "..."
    show debbie 81
    deb "{b}[firstname]{/b}?"
    deb "Is everything okay?"
    show debbie 82
    show player 111
    player_name "Yes, {b}[deb_name]{/b}."
    show player 108
    player_name "I'm just having trouble sleeping..."
    show player 109
    show debbie 81
    deb "Oh..."
    deb "That's okay, sweetie."
    deb "I understand what you're going through..."
    show debbie 83
    deb "... And with everything that's happened..."
    show debbie 82
    show player 108
    player_name "Can I sleep with you tonight?"
    show player 109
    show debbie 83
    deb "{b}*Sigh*{/b}"
    show debbie 81
    show player 110
    deb "Yeah, that should be okay. So long as you wake up early and return to your room before {b}[jen_name]{/b} wakes up."
    show debbie 82
    show player 111
    player_name "I can do that."
    scene debbie_cuddle
    with fade
    show debbies 77
    with fade
    player_name "Thanks, {b}[deb_name]{/b}."
    show debbies_bed_overlay zorder 3 at Position(xpos=438)
    show debbies 79 zorder 2 at Position(xpos=365)
    with dissolve
    player_name "I like being close to you..."
    player_name "... Here in your bed."
    show debbies 80
    deb "I know, sweetie. I like it too."
    show debbies 79
    player_name "You do?"
    return

label mom_bed_sleep_together_cuddle_sex_available:
    show debbies 79
    player_name "Can we cuddle?"
    show debbies 80
    deb "Of course."
    show debbies 83 with dissolve
    player_name "You smell really good, {b}[deb_name]{/b}..."
    show debbies 84
    deb "Thanks, sweetie."
    show debbies 81_82 with dissolve
    pause 4
    show debbies 81
    return

label mom_bed_sleep_together_cuddle:
    show debbies 79
    player_name "Can we cuddle? Just a little bit?"
    show debbies 80
    deb "Sure, sweetie."
    show debbies 81_82 with dissolve
    deb "( !!! )"
    deb "( He's... caressing my hips... )"
    deb "( Is this too much? )"
    deb "( It feels good but should I stop him? )"
    show debbies 83
    player_name "You smell really good, {b}[deb_name]{/b}..."
    show debbies 82
    deb "( ... )"
    show debbies 84
    deb "Thanks, {b}[firstname]{/b}."
    show debbies 82
    return

label mom_bed_sleep_together_cuddle_keep_going:
    show debbies 81_82
    pause 4
    show debbies 81
    return

label mom_bed_sleep_together_kiss_sex_available:
    show debbies 83
    player_name "Can we kiss?"
    show debbies 84
    deb "You don't even have to ask, sweetie."
    deb "Come here..."
    show debbies 85_86 with dissolve
    pause 4
    show debbies 81 with dissolve
    return

label mom_bed_sleep_together_kiss:
    show debbies 83
    player_name "{b}[deb_name]{/b}..."
    show debbies 84
    deb "Yes?"
    show debbies 83
    player_name "Can I kiss you?"
    show debbies 84
    deb "You want to kiss me?"
    deb "... Right now?"
    show debbies 83
    player_name "Y-yes, please."
    show debbies 81
    deb "..."
    show debbies 83
    player_name "I don't know why... I just really want to kiss you right now."
    show debbies 84
    deb "... That's sweet, {b}[firstname]{/b}."
    show debbies 83
    player_name "So is it alright?"
    player_name "... to kiss you, just a little bit?"
    show debbies 84
    deb "I..."
    deb "I suppose we can kiss but just a little bit!"
    show debbies 83
    player_name "Y-yeah."
    show debbies 85_86 with hpunch
    deb "( !!! )"
    deb "( I can feel his tongue! )"
    deb "( Why is he so good at this?! )"
    deb "Mmm..."
    deb "( Oh my... )"
    deb "( His hands are on my breasts... )"
    deb "Mmm..."
    deb "( ... This is making me so horny! )"
    show debbies 83 with dissolve
    player_name "... How was that?"
    show debbies 84
    deb "..."
    deb "{b}*Gulp*{/b} That was really good, {b}[firstname]{/b}."
    show debbies 83
    player_name "Are you okay, {b}[deb_name]{/b}?"
    show debbies 84
    deb "... Yeah, I'm fine, sweetie. It's just hot in here."
    show debbies 81 with dissolve
    return

label mom_bed_sleep_together_kiss_keep_going:
    show debbies 85_86 with dissolve
    pause 4
    show debbies 81 with dissolve
    return

label mom_bed_sleep_together_breastfeed_sex_available:
    show debbies 83
    player_name "Can I... suck on your breasts?"
    show debbies 87 with dissolve
    deb "Just... be gentle."
    show debbies 88_89 at Position(xpos=329) with dissolve
    pause 4
    show debbies 89 at Position(xpos=327)
    return

label mom_bed_sleep_together_breastfeed:
    show debbies 83
    player_name "Can I... Kiss you down here?"
    show debbies 84
    deb "Hmm?"
    show debbies 83
    player_name "Your neck... And your chest..."
    show debbies 84
    deb "Oh, I dunno..."
    show debbies 83
    player_name "Just a little, I promise."
    show debbies 84
    deb "..."
    show debbies 87 with dissolve
    deb "... Be gentle."
    show debbies 88_89 at Position(xpos=329) with dissolve
    deb "( Ah! He's sucking my nipple! )"
    deb "( ... Oh God! )"
    deb "( Ahhh...!! )" with hpunch
    deb "( It feels so good! )"
    deb "( ... )"
    deb "( He's latched on so tight! )"
    deb "( ... I wish I could take this further! )"
    show debbies 88 at Position(xpos=329)
    return

label mom_bed_sleep_together_breastfeed_keep_going:
    show debbies 88_89 at Position(xpos=329) with dissolve
    pause 4
    show debbies 88 at Position(xpos=329)
    return

label mom_bed_sleep_together_rub_sex_available:
    show debbies 90 at Position(xpos=327) with dissolve
    deb "Yes, sweetie."
    show debbies 91 with dissolve
    deb "Take it out, and place it between my legs..."
    show debbies 92 with dissolve
    pause
    show debbies 93_94 at Position(xpos=329) with dissolve
    deb "You're so nice and hard, now."
    deb "I can feel it..."
    deb "Keep moving your hips, sweetie."
    deb "It feels so good..."
    deb "Ahhh.."
    pause
    show debbies 93_94 at Position(xpos=329)
    pause 4
    show debbies 94 at Position(xpos=327)
    return

label mom_bed_sleep_together_rub:
    show debbies 90 at Position(xpos=327) with dissolve
    deb "( What is he... )"
    show debbies 91 with dissolve
    deb "..."
    show debbies 92 with dissolve
    deb "( Did he just take it out?! )"
    deb "( Oh my god! )"
    deb "( I should be stopping him but... )"
    deb "( ... )"
    show debbies 93_94 at Position(xpos=329) with hpunch
    deb "( Ahhh...! )"
    deb "Mmm!"
    deb "( I'm getting close! )"
    deb "( He's going to make me cum! )"
    deb "( OH GOD!!! )"
    deb "( I'm gonna... )"
    deb "( I'M GONNA!! )"
    deb "AAAHHH!"
    show debbies 93 at Position(xpos=329)
    return

label mom_bed_sleep_together_rub_keep_going:
    show debbies 93_94 at Position(xpos=329) with dissolve
    pause 4
    show debbies 93 at Position(xpos=329)
    return

label mom_bed_sleep_together_stop:
    show debbies 79 with dissolve
    return

label mom_bed_sleep_together_fuck:
    show debbies 95 with dissolve
    if randomizer() <= 50:
        deb "Let me help you, sweetie..."
        deb "...Now, slide it inside me... deep."
        show debbies 96 with dissolve
        deb "Just like that... Yes, sweetie..."
    else:
        deb "I can't take all the teasing like that."
        deb "I want you to bury that big cock in me."
        deb "Don't go slow..."
        deb "You've teased me enough."
        show debbies 96 with dissolve
        pause
    show debbies 98 at Position(xpos=327) with dissolve
    if randomizer() <= 50:
        deb "AHH!"
    else:
        deb "Ohh..."
    show screen xray_scr
    pause
    if anim_toggle:
        hide screen xray_scr
        show debbies 97_98 at Position(xpos=329)
        deb "Oh, yes!{p=1}{nw}"
        deb "Yes, sweetie...{p=1}{nw}"
        deb "Keep going...{p=1}{nw}"
        deb "Deeper... Faster!!{p=1}{nw}"
    else:

        hide screen xray_scr
        show debbies 97 at Position(xpos=329)
        deb "Oh, yes!"
        show debbies 98 at Position(xpos=327)
        deb "Yes, sweetie..."
        show debbies 97 at Position(xpos=329)
        deb "Keep going..."
        show debbies 98 at Position(xpos=327)
        deb "Deeper... Faster!!"
        show debbies 97 at Position(xpos=329)
    show debbies 97 at Position(xpos=329)
    return

label mom_bed_sleep_together_fuck_keep_going:
    show screen xray_scr 
    pause
    hide screen xray_scr 
    $ animcounter = 0
    while animcounter < 4:
        if anim_toggle:
            if not renpy.showing("debbies 97_98"):
                show debbies 97_98 as debbies at Position(xpos = 329)
            pause 4
        else:

            $ pose_counter = 0
            $ pose_list = [97,98]
            $ xpos_list = [329,327]
            $ poses_done = []
            while poses_done != pose_list:
                show expression "debbies {}".format(pose_list[pose_counter]) as debbies at Position(xpos = xpos_list[pose_counter])
                pause
                $ poses_done.append(pose_list[pose_counter])
                $ pose_counter += 1
        $ animcounter += 1
    return

label mom_bed_sleep_together_fuck_cum:
    deb "AAHH!!!"
    show white zorder 4
    show debbies 99 at Position(xpos=327) with vpunch
    hide white with dissolve
    if randomizer() <= 50:
        deb "YES baby..."
        $ xray = False
        show debbies 98 at Position(xpos=327) with dissolve
        deb "{b}*Huffing*{/b}"
        deb "Stay inside me a bit longer..."
        deb "I like feeling your cum inside me."
        show debbies 100 with fade
        deb "You came so much..."
        player_name "Was it... wrong?"
        deb "No, sweetie. It felt wonderful!"
    else:
        $ xray = False
        show debbies 98 at Position(xpos=327) with dissolve
        deb "Oh, baby..."
        show debbies 100 with fade
        deb "I didn't think you were going to stop..."
        deb "That was great..."

    show debbies 80 at Position(xpos=329) with fade
    if randomizer() <= 50:
        deb "Please stay with me tonight."
    else:
        deb "Are you sleeping here tonight?"
    show debbies 79
    return

label mom_bed_sleep_together_fuck_cum_stay:
    player_name "Alright, {b}[deb_name]{/b}."
    show debbies 80
    deb "Good boy."
    hide debbies
    hide debbies_bed_overlay
    scene black
    with fade
    return

label mom_bed_sleep_together_fuck_cum_leave:
    player_name "I can't. Maybe tomorrow."
    show debbies 80
    deb "Aww... Well don't be shy."
    scene black
    hide debbies
    hide debbies_bed_overlay
    with fade
    pause
    scene debbie_bedroom_closeup
    show player 111 at right
    show debbie 82 at left
    with dissolve
    player_name "Goodnight, {b}[deb_name]{/b}."
    show player 110
    show debbie 81
    deb "Goodnight, sweetie."
    deb "I love you."
    show debbie 82
    show player 111
    player_name "Love you too, {b}[deb_name]{/b}."
    hide player
    hide debbie
    with dissolve
    return

label mom_bed_sleep_together_sleep:
    show debbies 79 with dissolve
    player_name "Goodnight, {b}[deb_name]{/b}..."
    show debbies 80 with dissolve
    deb "Goodnight, {b}[firstname]{/b}..."
    hide debbies
    hide debbies_bed_overlay
    return

label mom_bed_night:
    scene debbie_bedroom_night
    show player 106 with dissolve
    player_name "( Should I sneak into her bed? )"
    show player 34
    player_name "Hmm..."
    player_name "( I shouldn't bother her when she's sleeping... )"
    hide player with dissolve
    return

label mom_bed_day:
    scene debbie_bedroom
    show player 1 with dissolve
    player_name "( It felt so good, jerking off in her bed. )"
    player_name "( ... Just utterly enveloped in her scent. )"
    show player 5
    player_name "( But I promised her I wouldn't do that again. )"
    player_name "( I should really try my hardest to keep that promise. )"
    hide player with dissolve
    return

label mom_sleeping:
    call expression game.dialog_select("mom_sleeping_pre") from _call_expression_59
    $ Sleep()
    if M_mom.is_set("sex available"):
        if randomizer() <= 50:
            call expression game.dialog_select("mom_sleeping_sex_available_random") from _call_expression_60
        else:

            call expression game.dialog_select("mom_sleeping_sex_available") from _call_expression_61
    jump expression game.dialog_select("mom_bedroom")

label mom_sleeping_pre:
    scene location_home_debbiebedroom_night_sleep with fade
    show unlock11 at truecenter with dissolve
    pause
    hide unlock11 with dissolve
    scene black with fade
    hide location_home_debbiebedroom_night_sleep with fade
    return

label mom_sleeping_sex_available_random:
    scene debbie_bedroom
    show player 7 with dissolve
    player_name "{b}*Yawn*{/b}"
    show player 101 with dissolve
    player_name "Looks like {b}[deb_name]{/b} is already up."
    show player 8 with dissolve
    pause
    show player 14 with dissolve
    player_name "I'd better sneak out of here, so {b}[jen_name]{/b} doesn't see me."
    hide debbie
    hide player
    with dissolve
    return

label mom_sleeping_sex_available:
    scene debbie_bedroom_closeup2
    show debbie 83 at left
    show player 7 at right
    with dissolve
    player_name "{b}*Yawn*{/b}"
    show player 8 with dissolve
    show debbie 81
    deb "Morning, {b}[firstname]{/b}."
    show debbie 82
    show player 22 with hpunch
    player_name "!!!"
    show player 108 with dissolve
    player_name "Morning! Did I wake you?"
    show player 109
    show debbie 82
    deb "No, sweetie."
    deb "I, too, was enjoying you next to me."
    show debbie 81
    show player 111
    player_name "Yeah, it was nice sleeping with you. Your bed has softer pillows."
    show player 110
    show debbie 81
    deb "Right."
    deb "I suppose I'd better get up too."
    deb "Thanks again for sleeping with me."
    show debbie 82
    show player 111
    player_name "You're welcome. Love you, {b}[deb_name]{/b}."
    show player 110
    show debbie 81
    deb "I love you too, sweetie."
    hide debbie
    hide player
    with dissolve
    return

label mom_room_laundry_dialogue:
    scene debbie_bedroom
    show player 276 with dissolve
    player_name "I should take this down to the {b}basement{/b}."
    hide player with dissolve
    return

label mom_room_laundry:
    call expression game.dialog_select("mom_room_laundry_dialogue") from _call_expression_62
    $ M_mom.trigger(T_mom_got_laundry)
    $ game.main()

label mom_sex_replay:
    $ M_mom.set("sex available", True)
    scene debbie_bedroom_closeup2
    $ rnd = randomizer()
    if rnd < 50:
        jump expression game.dialog_select("sex_mom_bed_intro_3")

    elif rnd < 75:
        jump expression game.dialog_select("sex_mom_bed_intro_2")
    jump expression game.dialog_select("sex_mom_bed_intro_1")

label mom_sex:
    $ M_mom.set("sex speed", .4)
    $ M_mom.set("change angle", False)
    $ mom_sex_position = "missionary"
    $ cum = False
    $ anim_toggle = False
    $ xray = False
    $ first_time_cowgirl = True
    $ first_time_suck_tits = True
    call expression game.dialog_select("mom_sex_pre") from _call_expression_63
    jump expression game.dialog_select("mom_sex_loop")

label mom_sex_pre:
    show debbies 106 at left
    with fade
    deb "Now, start nice and slow, sweetie."
    show debbies 103 with dissolve
    pause
    show debbies 104 with dissolve
    deb "Aahh..."
    show debbies 104 at left
    return

label mom_sex_loop_pre:
    if mom_sex_position in ["missionary", "suck tits"]:
        if not renpy.showing("debbie_bedroom_closeup_sex"):
            scene debbie_bedroom_closeup_sex

        if mom_sex_position == "missionary":
            show debbies 106 at left

        if mom_sex_position == "suck tits":
            if first_time_suck_tits and randomizer() <= 50:
                call expression game.dialog_select("mom_sex_loop_pre_suck_tits") from _call_expression_64
                $ first_time_suck_tits = False

    elif mom_sex_position == "cowgirl":
        if not renpy.showing("bedroom_sex_05") and M_mom.is_set("change angle"):
            scene bedroom_sex_05
            show debbies 170

        elif not renpy.showing("debbie_bedroom_closeup_sex"):
            scene debbie_bedroom_closeup_sex
            show debbies 62 at left
        else:

            show debbies 62 at left

        if first_time_cowgirl:
            call expression game.dialog_select("mom_sex_loop_pre_cowgirl") from _call_expression_65
            $ first_time_cowgirl = False
    jump expression game.dialog_select("mom_sex_loop")

label mom_sex_loop_pre_suck_tits:
    show debbies 67 at left with dissolve
    player_name "Can I kiss your breasts?."
    show debbies 68
    deb "Be gentle."
    deb "They're sensitive."
    return

label mom_sex_loop_pre_cowgirl:
    with dissolve
    if randomizer() <= 50:
        deb "Lay back and let me take a turn."
    else:
        deb "You just like watching my tits bounce don't you, my naughty boy?"
    return

label mom_sex_loop:
    show screen xray_scr 
    pause
    hide screen xray_scr 
    $ animcounter = 0
    while animcounter < 4:
        if anim_toggle:
            if mom_sex_position == "missionary":
                show expression AnimatedImage("debbies", [103,104,105,104], M_mom) as debbies at left

            elif mom_sex_position == "suck tits":
                if not renpy.showing("debbies 68_67"):
                    show debbies 68_67 as debbies at left

            elif mom_sex_position == "cowgirl":
                if M_mom.is_set("change angle"):
                    show expression AnimatedImage("debbies", [170,171,172,173,174,175,176,177], M_mom) as debbies
                else:

                    show expression AnimatedImage("debbies", [65,66,64], M_mom) as debbies at left

            pause 5
            call expression game.dialog_select("debbie_bed_hscene_dialog") from _call_expression_66
            pause 3
        else:

            $ M_mom.set("sex poses", [])
            $ pose_counter = 0
            if mom_sex_position == "missionary":
                $ pose_list = [103,104,105,104]

            elif mom_sex_position == "suck tits":
                $ pose_list = [68,67]

            elif mom_sex_position == "cowgirl":
                if M_mom.is_set("change angle"):
                    $ pose_list = [170,171,172,173,174,175,176,177]
                else:

                    $ pose_list = [65,66,64]
            $ poses_done = []
            while poses_done != pose_list:
                show expression "debbies {}".format(pose_list[pose_counter]) as debbies at left
                pause
                $ poses_done.append(pose_list[pose_counter])
                $ pose_counter += 1
            call expression game.dialog_select("debbie_bed_hscene_dialog") from _call_expression_67
        $ animcounter += 1
    call screen mom_sex_options

label debbie_bed_hscene_dialog:
    if mom_sex_position == "missionary":
        if randomizer() <= 33:
            if animcounter == 1:
                deb "Ahhhh!!!{p=1}{nw}"

            elif animcounter == 3:
                deb "Oh, sweetie!!!{p=1}{nw}"
                player_name "Uhhh...{p=1}{nw}"

        elif randomizer() <= 66:
            if animcounter == 1:
                deb "Oh...{p=1}{nw}"

            elif animcounter == 2:
                deb "Give it to me, sweetie.{p=2}{nw}"
                player_name "You like that?{p=2}{nw}"
                deb "Yesss....{p=1}{nw}"

            elif animcounter == 3:
                deb "Ahh!{p=1}{nw}"
        else:

            if animcounter == 2:
                deb "Faster, sweetie!{p=2}{nw}"
                deb "I'm almost there!{p=2}{nw}"
                player_name "Oh, {b}[deb_name]{/b}! I can feel you clenching down on me!{p=2}{nw}"

            elif animcounter == 3:
                deb "Ahh!!!{p=1}{nw}"

    elif mom_sex_position == "suck tits":
        if randomizer() <= 33:
            if animcounter == 2:
                deb "Yes, sweetie....{p=1}{nw}"
                deb "Keep sucking on them.{p=1}{nw}"

        elif randomizer() <= 66:
            if animcounter == 1:
                deb "Mmmm...{p=1}{nw}"

            elif animcounter == 2:
                deb "Oh!{p=1}{nw}"
                deb "Gently, sweetie!{p=1}{nw}"
        else:
            if animcounter == 3:
                deb "Oh, baby...{p=1}{nw}"
                deb "You like sucking on my titties?{p=2}{nw}"
                player_name "Mmhmm..{p=1}{nw}"

    elif mom_sex_position == "cowgirl":
        if randomizer() <= 50:
            if animcounter == 1:
                deb "Ahhhh!!!{p=1}{nw}"

            elif animcounter == 3:
                deb "Oh, sweetie!!!{p=1}{nw}"
                player_name "Uhhh...{p=1}{nw}"
        else:

            if animcounter == 2:
                deb "You like this?{p=2}{nw}"
                player_name "Yeah...{p=1}{nw}"
                deb "I do too...{p=2}{nw}"
                deb "I can really feel you deep inside me like this.{p=2}{nw}"

            elif animcounter == 3:
                deb "Ahh!!!{p=1}{nw}"
                deb "Oh, baby...{p=1}{nw}"
    return

label mom_sex_cum_inside:
    if mom_sex_position == "missionary":
        if randomizer() <= 50:
            call expression game.dialog_select("mom_sex_cum_inside_missionary_pre_random") from _call_expression_68
            $ cum = True
            call expression game.dialog_select("mom_sex_cum_inside_missionary_after_cum_random") from _call_expression_69
            $ xray = False
            call expression game.dialog_select("mom_sex_cum_inside_missionary_after_xray_random") from _call_expression_70
        else:

            call expression game.dialog_select("mom_sex_cum_inside_missionary_pre") from _call_expression_71
            $ cum = True
            call expression game.dialog_select("mom_sex_cum_inside_missionary_after_cum") from _call_expression_72
            $ xray = False
            call expression game.dialog_select("mom_sex_cum_inside_missionary_after_xray") from _call_expression_73
    else:

        if not renpy.showing("debbie_bedroom_closeup_sex"):
            scene debbie_bedroom_closeup_sex

        call expression game.dialog_select("mom_sex_cum_inside_cowgirl_pre") from _call_expression_74
        $ cum = True
        call expression game.dialog_select("mom_sex_cum_inside_cowgirl_after_cum") from _call_expression_75
    jump expression game.dialog_select("mom_bed_after_sex")

label mom_sex_cum_inside_missionary_pre_random:
    show debbies 103 at left
    player_name "{b}[deb_name]{/b}, I'm gonna..."
    deb "Let it out, sweetie!"
    show debbies 105 with hpunch
    player_name "{b}[deb_name]{/b}!!!"
    show white with dissolve
    return

label mom_sex_cum_inside_missionary_pre:
    show debbies 105 at left with hpunch
    player_name "Oh, {b}[deb_name]{/b}!"
    show white with dissolve
    return

label mom_sex_cum_inside_missionary_after_cum_random:
    hide white with dissolve
    deb "Ahhh!!!"
    deb "Yes!"
    deb "Stay inside me for a while..."
    show debbies 106 with fade
    return

label mom_sex_cum_inside_missionary_after_cum:
    hide white with dissolve
    deb "Ahh!!!"
    deb "Oh, sweetie..."
    deb "That was intense..."
    player_name "That felt good."
    deb "Yeah..."
    deb "Oh, sweetie. You are good."
    show debbies 106 with fade
    return

label mom_sex_cum_inside_missionary_after_xray_random:
    deb "You let out so much..."
    return

label mom_sex_cum_inside_missionary_after_xray:
    deb "You're still so stiff..."
    deb "I bet you could go again!"
    return

label mom_sex_cum_inside_cowgirl_pre:
    scene debbie_bedroom_closeup_sex
    show debbies 69 at left with vpunch
    player_name "{b}[deb_name]{/b}!!!"
    deb "Ahh!!!"
    show white with dissolve
    return

label mom_sex_cum_inside_cowgirl_after_cum:
    hide white with dissolve
    pause
    deb "Oh..."
    show debbies 70 with dissolve
    deb "You should warn me when you're about to do it, sweetie."
    player_name "Sorry, {b}[deb_name]{/b}."
    player_name "I just couldn't help it..."
    player_name "It felt too good..."
    deb "It's okay. Let's clean you up..."
    return

label mom_sex_cum_outside:
    if mom_sex_position == "missionary":
        if randomizer() <= 50:
            call expression game.dialog_select("mom_sex_cum_outside_missionary_pre_random") from _call_expression_76
            $ xray = False
            call expression game.dialog_select("mom_sex_cum_outside_missionary_after_xray_random") from _call_expression_77
        else:

            call expression game.dialog_select("mom_sex_cum_outside_missionary_pre") from _call_expression_78
            $ xray = False
            call expression game.dialog_select("mom_sex_cum_outside_missionary_after_xray") from _call_expression_79
    else:

        if not renpy.showing("debbie_bedroom_closeup_sex"):
            scene debbie_bedroom_closeup_sex

        if randomizer() <= 50:
            call expression game.dialog_select("mom_sex_cum_outside_cowgirl_random") from _call_expression_80
        else:

            call expression game.dialog_select("mom_sex_cum_outside_cowgirl") from _call_expression_81
    jump expression game.dialog_select("mom_bed_after_sex")

label mom_sex_cum_outside_missionary_pre_random:
    show debbies 103
    player_name "{b}[deb_name]{/b}, I'm gonna..."
    deb "Let it out, sweetie!"
    return

label mom_sex_cum_outside_missionary_pre:
    show debbies 103
    player_name "{b}[deb_name]{/b}, can I cum on top of you?"
    deb "Yes, sweetie!"
    return

label mom_sex_cum_outside_missionary_after_xray_random:
    show debbies 107 with dissolve
    show white with dissolve
    show playersex 70 at Position(xpos=568, ypos=348)
    hide white with dissolve
    pause
    show playersex 71 at Position(xpos=612, ypos=450)
    pause
    hide playersex
    show debbies 108
    deb "Oh!!!"
    deb "Wow!"
    deb "Heh, look at me! I'm covered!"
    player_name "Yeah... I guess I got a little carried away."
    return

label mom_sex_cum_outside_missionary_after_xray:
    show debbies 107 with dissolve
    show white with dissolve
    show playersex 70 at Position(xpos=568, ypos=348)
    hide white with dissolve
    pause
    show playersex 71 at Position(xpos=612, ypos=450)
    pause
    hide playersex
    show debbies 108
    deb "Oh!"
    deb "I didn't think it'd be that much!"
    deb "I'm going to have to change the sheets!"
    player_name "Sorry..."
    deb "Don't be. It feels wonderful!"
    return

label mom_sex_cum_outside_cowgirl_random:
    show debbies 64 at left
    player_name "{b}[deb_name]{/b}, I'm gonna..."
    show debbies 109 with dissolve
    show white with dissolve
    show playersex 72 at Position(xpos=478, ypos=418)
    hide white with dissolve
    pause
    show playersex 73 at Position(xpos=524, ypos=478) with dissolve
    deb "Thanks for the warning, sweetie."
    deb "Let's get ourselves cleaned up..."
    return

label mom_sex_cum_outside_cowgirl:
    show debbies 64 at left
    player_name "{b}[deb_name]{/b}! I'm..."
    show debbies 109 with dissolve
    show white with dissolve
    show playersex 72 at Position(xpos=478, ypos=418)
    hide white with dissolve
    pause
    show playersex 73 at Position(xpos=524, ypos=478) with dissolve
    deb "!!!"
    player_name "Wow, that's a lot..."
    deb "Ha Ha Ha..."
    deb "It's everywhere!."
    deb "I'm soaked!"
    return

label mom_bed_after_sex:
    call expression game.dialog_select("mom_bed_after_sex_pre") from _call_expression_82
    if randomizer() <= M_mom.get("mom concerned"):
        if M_mom.get("mom concerned") > 0:
            $ M_mom.set("mom concerned", M_mom.get("mom concerned") - 20)

        if M_mom.get("mom concerned") < 0:
            $ M_mom.set("mom concerned", 0)

        call expression game.dialog_select("mom_bed_after_sex_random") from _call_expression_83
    call expression game.dialog_select("mom_bed_after_sex_after") from _call_expression_84
    $ renpy.end_replay()
    $ persistent.cookie_jar["Debbie"]["unlocked"] = True
    $ persistent.cookie_jar["Debbie"]["gallery"]["11_unlocked"] = True

    $ game.timer.tick()
    $ mom_dialogue_advance = True
    $ game.main()

label mom_bed_after_sex_pre:
    scene debbie_bedroom
    show player 227 at Position(xpos=200)
    show debbie 73 at Position(xpos=650)
    with fade
    deb "Sweetie..."
    return

label mom_bed_after_sex_random:
    deb "I want you to tell me if you don't want to do this anymore, okay?"
    show player 228
    show debbie 76
    player_name "No, it's fine, {b}[deb_name]{/b}."
    player_name "I always feel like doing it with you."
    show player 227
    show debbie 77
    deb "Really?"
    show player 228
    player_name "Yeah, that's all I think about doing when I see you..."
    show player 227
    show debbie 75
    deb "You're always so sweet..."
    return

label mom_bed_after_sex_after:
    deb "Give me a kiss."
    hide player
    show debbie 80
    with dissolve
    pause
    show debbie 79 with dissolve
    pause
    show debbie 80 with dissolve
    pause
    show player 228 at Position(xpos=200)
    show debbie 78
    with dissolve
    player_name "I love you, {b}[deb_name]{/b}!"
    show player 227
    show debbie 75
    deb "I love you too, sweetie..."
    hide debbie
    hide player
    with dissolve
    return
# Decompiled by unrpyc: https://github.com/CensoredUsername/unrpyc
