label car_dialogue:
    if M_mom.is_state(S_mom_mall_outing):
        call expression game.dialog_select("garage_car_mom_mall_outing") from _call_expression_596
    ...
    if M_mom.is_state(S_mom_check_car):
        ...
        call expression game.dialog_select("garage_car_mom_check_car") from _call_expression_597
        $ player.location.call_screen(False)
    else:

        if seen_garage_locked:
            call expression game.dialog_select("garage_car_seen") from _call_expression_598
        else:
            call expression game.dialog_select("garage_car_not_seen") from _call_expression_599
...
label engine_broken:
    call expression game.dialog_select("engine_broken_dialogue") from _call_expression_600
...
label lawnmower_dialogue:
    ...
    if M_mom.is_state(S_mom_fill_mower) and not player.has_item("gas_can"):
        if not game.timer.is_dark():
            call expression game.dialog_select("garage_mom_fill_mower_no_gas") from _call_expression_601
        else:
            call expression game.dialog_select("garage_mom_fill_mower_night") from _call_expression_602

    elif M_mom.is_state(S_mom_fill_mower) and player.has_item("gas_can"):
        if not game.timer.is_dark():
            call expression game.dialog_select("garage_mom_fill_mower_success") from _call_expression_603
            ...
            call expression game.dialog_select("garage_mom_fill_mower_night") from _call_expression_604
    else:
        call expression game.dialog_select("garage_lawnmower_not_needed") from _call_expression_605
    $ game.main()
...
label garage_use_workbench:
    if M_dewitt.is_state(S_dewitt_make_new_flute) and player.has_item("drill") and player.has_item("stick"):
        call expression game.dialog_select("garage_dewitt_make_new_flute") from _call_expression_606
        ...
        call expression game.dialog_select("garage_dewitt_make_replacement_guitar") from _call_expression_607
        ...
        call expression game.dialog_select("garage_build_easels") from _call_expression_608
        $ player.remove_item("wood_pile")
        $ player.get_item("easels")
        $ game.timer.tick()
    else:

        call expression game.dialog_select("garage_workbench_not_needed") from _call_expression_609
    $ game.main()

label garage_dewitt_drill:
    call expression game.dialog_select("garage_dewitt_drill_dialogue") from _call_expression_610
...
label debbie_car_sex:
    ...
    call expression game.dialog_select("debbie_car_sex_pre") from _call_expression_611
    if not M_mom.is_set("car sex"):
        call expression game.dialog_select("debbie_car_sex_pre_first_time") from _call_expression_612
        $ M_mom.set("car sex", True)
    else:

        call expression game.dialog_select("debbie_car_sex_pre_repeat") from _call_expression_613
    menu:
        "Jerk me off.":
            $ M_mom.set("car jerk", True)
            call expression game.dialog_select("debbie_car_sex_jerk_off") from _call_expression_614
            jump expression game.dialog_select("car_mom_sex_loop")
        "Blowjob.":

            $ M_mom.set("car jerk", False)
            call expression game.dialog_select("debbie_car_sex_bj") from _call_expression_615
            jump expression game.dialog_select("car_mom_sex_loop")

        "Leave." if store._in_replay == None:
            call expression game.dialog_select("debbie_car_sex_leave") from _call_expression_616
    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            if animcounter in [1,3]:
                call expression game.dialog_select("car_mom_hscene_dialog") from _call_expression_617
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            if animcounter in [1,3]:
                call expression game.dialog_select("car_mom_hscene_dialog") from _call_expression_618
        $ animcounter += 1
...
label car_mom_sex_cum:
    if M_mom.is_set("car jerk"):
        call expression game.dialog_select("car_mom_jerk_cum") from _call_expression_619
    else:
        call expression game.dialog_select("car_mom_bj_cum") from _call_expression_620
    scene black with fade
    $ renpy.end_replay()
    $ persistent.cookie_jar["Debbie"]["unlocked"] = True
    $ persistent.cookie_jar["Debbie"]["gallery"]["05_unlocked"] = True
