label home_front:
    ...
    if M_roxxy.is_state(S_roxxy_studying_at_mcs):
        call expression game.dialog_select("home_roxxy_studying_at_mcs") from _call_expression_921
        $ M_roxxy.trigger(T_roxxy_get_cheerleader)
        $ player.go_to(L_home_bedroom)
        $ game.main()

    elif M_roxxy.is_state(S_roxxy_cookies_and_milk):
        call expression game.dialog_select("home_front_roxxy_cookies_and_milk") from _call_expression_922
        $ game.timer.tick(1)
        ...
        call expression game.dialog_select("home_front_mom_mrsj_visit") from _call_expression_923
        $ M_mom.trigger(T_mom_mrsj_condolences)

    elif M_mom.is_state(S_mom_mow_lawn):
        call expression game.dialog_select("home_front_mom_mow_lawn") from _call_expression_924
        $ M_mom.trigger(T_mom_mowed_lawn)


    elif M_mom.is_state(S_mom_car_fixed):
        call expression game.dialog_select("home_front_mom_car_fixed") from _call_expression_925

        $ player.go_to(L_home_garage)
        call expression game.dialog_select("home_front_mom_car_fixed_check_car") from _call_expression_926
        $ M_mom.set("jerk count", 0)
        $ M_mom.set("sex speed", .3)
        menu:
            "A little longer.":
                call expression game.dialog_select("home_front_mom_car_fixed_check_car_little_longer") from _call_expression_927
                jump expression game.dialog_select("mom_car_jerk_loop")
            "Leave the car.":

                call expression game.dialog_select("home_front_mom_car_fixed_check_car_finished") from _call_expression_928
        ...
        call expression game.dialog_select("home_front_mom_bad_guys_revisit") from _call_expression_929
