...
label seasucc_dialogue:
    ...
    if M_aqua.is_state(S_aqua_seasucc_intro):
        call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_intro") from _call_expression_1684
        $ M_aqua.trigger(T_aqua_seasucc)

    elif M_aqua.is_state(S_aqua_seasucc_mushroom) and not player.has_item("mushroom"):
        call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_no_mushroom") from _call_expression_1685

    elif M_aqua.is_state(S_aqua_seasucc_mushroom) and player.has_item("mushroom"):
        $ player.remove_item("mushroom")
        call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_mushroom_intro") from _call_expression_1686
        jump expression game.dialog_select("seasucc_loop")
    else:

        label aqua_succ_replay:
            call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_mushroom_repeat_intro") from _call_expression_1687
        if not store._in_replay == None:
            jump expression game.dialog_select("succ_replay_jump")
        menu:
            "Yes.":
                label succ_replay_jump:
                    call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_mushroom_repeat_yes") from _call_expression_1688
                jump expression game.dialog_select("seasucc_loop")
            "No.":

                call expression game.dialog_select("seasucc_dialogue_aqua_seasucc_mushroom_repeat_no") from _call_expression_1689
    hide aqua
    hide player
    hide player_boner
    hide seasucc
...
label seasucc_loop:
    ...
    while animcounter < 4:
        if anim_toggle:
            show expression AnimatedImage("seasucc", [8,"8b","8c","8d","8e","8f","8g","8h","8i"], M_aqua) as seasucc at Position(xalign = 0.35, yalign = 0.0)
            pause 4
            call expression game.dialog_select("seasucc_hscene_dialog") from _call_expression_1690
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            call expression game.dialog_select("seasucc_hscene_dialog") from _call_expression_1691
        $ animcounter += 1
...
label seasucc_cum:
    call expression game.dialog_select("seasucc_cum_pre") from _call_expression_1692
    $ renpy.end_replay()
...
label aqua_lure_steal:
    call expression game.dialog_select("aqua_lure_steal_pre") from _call_expression_1693
    $ player.remove_item("special_lure")
    $ M_aqua.trigger(T_aqua_lure_steal)
    label follow_aqua:
        call expression game.dialog_select("aqua_lure_steal_after") from _call_expression_1694
    menu:
        "Dive!":
            call expression game.dialog_select("aqua_lure_steal_dive_pre") from _call_expression_1695
            $ playSound()
            call expression game.dialog_select("aqua_lure_steal_dive_after") from _call_expression_1696
        ...
        "Not yet.":
            ...
            call expression game.dialog_select("aqua_lure_steal_not_yet") from _call_expression_1697
    $ game.main()
