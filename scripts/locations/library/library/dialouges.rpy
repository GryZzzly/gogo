...
label library_jane_intro:
    call expression game.dialog_select("library_jane_intro_pre") from _call_expression_1134
    menu:
        "Sure." if player.has_money(20):
            call expression game.dialog_select("library_jane_intro_sure") from _call_expression_1135
            $ player.spend_money(20)
            $ M_player.set("library subscription", True)
            $ M_jane.trigger(T_jane_library_pass)
        "I'll pass.":

            call expression game.dialog_select("library_jane_intro_not_yet") from _call_expression_1136
            $ player.go_to(L_map)
...
label poem_assignment_lock:
    if M_bissette.is_state(S_bissette_find_poem_reference_book) and player.location.is_here(M_mia):
        call expression game.dialog_select("poem_assignment_lock_bissette_find_poem_reference_book") from _call_expression_1137
    else:

        call expression game.dialog_select("poem_assignment_lock_bissette_reference_book_search") from _call_expression_1138
    $ game.main()
...
label kamasutra:
    ...
    call expression game.dialog_select("kamasutra_dialogue") from _call_expression_1139
...
label french_dictionary:
    ...
    call expression game.dialog_select("french_dictionary_dialogue") from _call_expression_1140
...
label library_old_book:
    ...
    call expression game.dialog_select("library_old_book_dialogue") from _call_expression_1141
    $ player.location.call_screen(ui = False)
...
label breeding_guide:
    ...
    call expression game.dialog_select("breeding_guide_dialogue") from _call_expression_1142
    $ player.location.call_screen(ui = False)
