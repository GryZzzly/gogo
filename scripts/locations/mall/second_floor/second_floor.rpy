...
label photo_booth_dialogue:
    ...
    if M_roxxy.is_state(S_roxxy_fake_id_ask_terry) and M_roxxy.get("take roxxy mall"):
        call expression game.dialog_select("photo_booth_roxxy_take_picture") from _call_expression_501
        ...
        if not L_mall_photobooth.visited:
            call expression game.dialog_select("photo_booth_first_visit") from _call_expression_502
        call expression game.dialog_select("photo_booth_generic_dialogue") from _call_expression_503
    $ player.go_to(L_mall_floor2)
    $ game.main()
