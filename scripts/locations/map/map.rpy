label map_dialogue:
    ...
    if not game.timer.is_dark():
        ...
        if June.started(june_mc_help):
            call expression game.dialog_select("map_june_mc_help_started") from _call_expression_936
            $ june_mc_help.finish()

        elif June.started(june_erik_help):
            call expression game.dialog_select("map_june_erik_help_started") from _call_expression_937
            $ june_erik_help.finish()

        elif erik.in_progress(erik_intro):
            call expression game.dialog_select("map_erik_intro_in_progress") from _call_expression_938
            $ L_diane_yard.unlock()
            $ L_mall.unlock()
            $ L_beach.unlock()
            $ L_treehouse.unlock()
