label map_lock_check(destination_screen, destination_label):
    ...
    if M_dewitt.is_state(S_dewitt_eve_karaoke) and destination_screen == "Erik":
        call map_lock_check_destination_jump (jump_dest_screen, jump_dest_label) from _call_map_lock_check_destination_jump

    elif M_mia.get_state() in [S_mia_midnight_help, S_mia_locked_room] and destination_screen == "Mia":
        call map_lock_check_destination_jump (jump_dest_screen, jump_dest_label) from _call_map_lock_check_destination_jump_1

    elif not M_mia.is_set("church night locked") and game.timer.is_evening() and destination_screen == "Church":
        call map_lock_check_destination_jump (jump_dest_screen, jump_dest_label) from _call_map_lock_check_destination_jump_2
        ...
        call map_lock_check_destination_jump (jump_dest_screen, jump_dest_label) from _call_map_lock_check_destination_jump_3
        ...
        if destination_screen == "School":
            call expression game.dialog_select("school_no_master_key_locked") from _call_expression_932
        else:

            call expression game.dialog_select("night_locked") from _call_expression_933
        ...
        scene expression game.timer.image("townmap{}")
        if destination_screen == "School":
            call expression game.dialog_select("school_no_master_key_locked") from _call_expression_934
        else:

            call expression game.dialog_select("night_locked") from _call_expression_935
    else:

        call map_lock_check_destination_jump (jump_dest_screen, jump_dest_label) from _call_map_lock_check_destination_jump_4
    $ game.main()
