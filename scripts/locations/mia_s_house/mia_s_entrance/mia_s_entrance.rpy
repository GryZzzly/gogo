label mias_entrance:
    ...
    if M_mia.is_state(S_mia_parent_blocking):
        call expression game.dialog_select("mias_entrance_mia_parent_blocking") from _call_expression_745
        ...
        call expression game.dialog_select("mias_entrance_mia_helen_fight") from _call_expression_746
        $ M_mia.trigger(T_mias_request)

    elif M_mia.is_state(S_mia_helen_refusal):
        call expression game.dialog_select("mias_entrance_mia_helen_refusal") from _call_expression_747
        $ M_mia.trigger(T_mia_church_mention)


    elif M_mia.is_state(S_mia_urgent_help):
        call expression game.dialog_select("mias_entrance_mia_urgent_help") from _call_expression_748
        $ M_mia.trigger(T_harold_missing)

    elif M_mia.is_state(S_mia_harold_found_news):
        call expression game.dialog_select("mias_entrance_mia_harold_found_news") from _call_expression_749
        $ M_mia.trigger(T_mia_give_news)

    elif M_mia.is_state(S_mia_route_split):
        call expression game.dialog_select("mias_entrance_mia_route_split") from _call_expression_750
        $ M_mia.trigger(T_mia_family_reunion)

    elif M_helen.is_state([S_helen_route_split, S_helen_harold_moved_on]):
        call expression game.dialog_select("mias_entrance_helen_route_split") from _call_expression_751
        $ M_helen.trigger(T_helen_master_servant)

    elif M_mia.is_state(S_mia_unexpected_visit) and game.timer.is_afternoon():
        call expression game.dialog_select("mias_entrance_mia_unexpected_visit") from _call_expression_752
