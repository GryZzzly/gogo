label mias_upstairs:
    ...
    if M_mia.is_state([S_mia_consult, S_mia_impress_harold, S_mia_parent_unblock, S_mia_strip_aftermath, S_mia_midnight_call]) and game.timer.is_dark():
        call expression game.dialog_select("mias_upstairs_mia_parent_unblock") from _call_expression_959
        $ player.go_to(L_miahouse)

    elif M_mia.is_state([S_mia_midnight_help, S_mia_locked_room]):
        call expression game.dialog_select("mias_upstairs_mia_midnight_help") from _call_expression_960

    elif M_mia.is_state(S_mia_unexpected_visit) and game.timer.is_afternoon():
        call expression game.dialog_select("mias_upstairs_mia_unexpected_visit") from _call_expression_961

    elif M_helen.is_state(S_helen_aftersex_mia_suspicious):
        call expression game.dialog_select("mias_upstairs_helen_aftersex_mia_suspicious") from _call_expression_962
