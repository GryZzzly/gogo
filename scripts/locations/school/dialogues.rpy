...
label night_closed_school:
    ...
    if False:
        if M_erik.get("webcam help"):
            ...
            call expression game.dialog_select("school_erik_webcam_quest") from _call_expression_1214
            call expression game.dialog_select("school_erik_webcam_quest_sneak_in") from _call_expression_1215

            $ game.main()
        else:

            call expression game.dialog_select("school_erik_webcam_quest_need_help") from _call_expression_1216
    else:

        call expression game.dialog_select("school_closed") from _call_expression_1217
    call expression game.dialog_select("map_dialogue") from _call_expression_1218
...
label roxxy_locker_sex_loop:
    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            pause 5
            call expression game.dialog_select("roxxy_locker_hscene_dialog") from _call_expression_1219
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            call expression game.dialog_select("roxxy_locker_hscene_dialog") from _call_expression_1220
        $ animcounter += 1
...
label roxxy_locker_sex_cum:
    call expression game.dialog_select("roxxy_locker_sex_cum_pre") from _call_expression_1221
    if M_roxxy.get("roxxy locker sex"):
        call expression game.dialog_select("roxxy_locker_sex_cum_repeat") from _call_expression_1222
    else:

        call expression game.dialog_select("roxxy_locker_sex_cum_first") from _call_expression_1223
    $ renpy.end_replay()
