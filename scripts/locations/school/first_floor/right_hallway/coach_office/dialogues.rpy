    ...
    if player.has_picked_up_item("master_key"):
        if M_bissette.is_state(S_bissette_roxxy_pom_poms):
            if player.has_item("pompoms"):
                ...
                call expression game.dialog_select("coachs_locker_bissette_roxxy_pom_poms") from _call_expression_1443
        python:
        ...
        if M_bissette.is_state(S_bissette_roxxy_pom_poms):
            call expression game.dialog_select("coachs_locker_locked_bissette_roxxy_pom_poms") from _call_expression_1444
        else:

            call expression game.dialog_select("coachs_locker_locked") from _call_expression_1445
    $ game.main()
...
label coachs_office_roxxy_pom_poms:
    ...
    if game.timer.is_morning():
        call expression game.dialog_select("coachs_office_roxxy_pom_poms_right_time") from _call_expression_1446
    else:

        call expression game.dialog_select("coachs_office_roxxy_pom_poms_wrong_time") from _call_expression_1447
...
label coach_locker_pom_poms:
    show coach_locker
    player_name "Awesome!"
    call expression game.dialog_select("coach_locker_pom_poms_dialogue") from _call_expression_1448
...
label coachs_office_locker_hide_fail:
    call expression game.dialog_select("coachs_office_locker_hide_fail_dialogue") from _call_expression_1449
...
label coachs_office_locker_peeking:
    call expression game.dialog_select("coachs_office_locker_peeking_dialogue") from _call_expression_1450
