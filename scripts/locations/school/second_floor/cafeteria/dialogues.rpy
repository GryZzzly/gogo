...
label cafeteria_erik_favor_known:
    call expression game.dialog_select("cafeteria_erik_favor_known_intro") from _call_expression_2176
    menu:
        "I think so!" if erik.known(erik_favor_2):
            call expression game.dialog_select("cafeteria_erik_favor_known_found_replacement") from _call_expression_2177
            $ erik_favor_2.finish()
            $ M_kevin.set_default_locations([[L_gym, L_school_cafeteria, L_gym, L_NULL],
                                            [L_gym, L_gym, L_gym, L_NULL]])
        "Not yet.":

            call expression game.dialog_select("cafeteria_erik_favor_known_not_yet") from _call_expression_2178
    hide player
    hide kevin
    with dissolve
    return
