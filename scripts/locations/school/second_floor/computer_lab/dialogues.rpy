...
label printer_dialogue:
    ...
    if M_bissette.is_state([S_bissette_scan_missing_pages, S_bissette_fix_printer]):
        call expression game.dialog_select("printer_dialogue_bissette_scan_missing_pages") from _call_expression_382
        $ M_bissette.trigger(T_bissette_broken_printer)

    elif M_bissette.is_state(S_bissette_print_poem_assignment):
        call expression game.dialog_select("printer_dialogue_bissette_print_poem_assignment") from _call_expression_383
        $ M_bissette.trigger(T_bissette_print_assignment)
    else:

        call expression game.dialog_select("printer_dialogue_nothing") from _call_expression_384
    $ game.main()
