...
label coffee_pot_dialogue:
    if M_okita.is_state(S_okita_dose_smith):
        if game.timer.is_afternoon():
            call expression game.dialog_select("coffee_pot_dialogue_wrong_time") from _call_expression_2174

        elif game.timer.is_morning():
            call expression game.dialog_select("coffee_pot_dialogue_right_time") from _call_expression_2175
