    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            if animcounter in [1,3]:
                call expression game.dialog_select("bissette_hscene_dialog") from _call_expression_979
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            if animcounter in [1,3]:
                call expression game.dialog_select("bissette_hscene_dialog") from _call_expression_980
        $ animcounter += 1
...
label bissettes_office_sex_cum:
    if M_bissette.get("change angle"):
        call expression game.dialog_select("bissettes_office_sex_cum_chair_angle") from _call_expression_981
    else:
        call expression game.dialog_select("bissettes_office_sex_cum_desk_angle") from _call_expression_982

    call expression game.dialog_select("bissettes_office_sex_cum_scene_change") from _call_expression_983

    if M_bissette.is_state(S_bissette_wine_sampling):
        call expression game.dialog_select("bissettes_office_sex_cum_first") from _call_expression_984
    else:
        call expression game.dialog_select("bissettes_office_sex_cum_repeat") from _call_expression_985
