...
label dewitt_office_dialogue:
    ...
    if dewitt_office_first_visit and not game.timer.is_dark():
        call expression game.dialog_select("dewitts_office_first_visit") from _call_expression_918
        $ dewitt_office_first_visit = False

    if player.location.is_here(M_dewitt):
        if M_dewitt.is_state(S_dewitt_office_reward):
            call expression game.dialog_select("dewitts_office_dewitt_office_reward") from _call_expression_919
        ...
        call expression game.dialog_select("dewitts_office_night_lock") from _call_expression_920
