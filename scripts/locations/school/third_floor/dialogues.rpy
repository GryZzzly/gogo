label okita_office_door:
    ...
    if M_okita.is_set("office locked"):
        if player.has_item("keycode_note"):
            ...
            call expression game.dialog_select("okita_office_door_need_keycode") from _call_expression_1354
    else:

        if M_okita.is_state(S_okita_tired_from_belt):
            call expression game.dialog_select("okita_office_door_okita_tired") from _call_expression_1355
        else:
...
label annie_enter_office_dialogue:
    ...
    if not M_okita.is_set("talked to annie"):
        call expression game.dialog_select("smith_office_annie_guarding") from _call_expression_1356
        $ M_okita.set("talked to annie", True)
    else:

        call expression game.dialog_select("smith_office_annie_guarding_repeat") from _call_expression_1357
        if player.has_required_chr(7):
            call expression game.dialog_select("smith_office_annie_guarding_distract_pass") from _call_expression_1358
            $ player.go_to(L_school_smithoffice)
            $ game.main()
        else:

            call expression game.dialog_select("smith_office_annie_guarding_distract_fail") from _call_expression_1359
    $ game.main()
