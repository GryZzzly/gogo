...
label ross_hscene_start:
    ...
    if M_ross.is_state(S_ross_paint_with_body):
        call expression game.dialog_select("ross_office_hscene") from _call_expression_2125
        call expression game.dialog_select("ross_hscene_first_time") from _call_expression_2126
    else:
    ...
    while animcounter < 4:
        if anim_toggle:
            ...
            if animcounter in [1,3]:
                call expression game.dialog_select("ross_hscene_dialog") from _call_expression_2127
            pause 3
        else:
            ...
            while poses_done != pose_list:
                ...
                $ pose_counter += 1
            if animcounter in [1,3]:
                call expression game.dialog_select("ross_hscene_dialog") from _call_expression_2128
        $ animcounter += 1
...
label ross_office_ross_sex_cum:
    call expression game.dialog_select("ross_office_ross_sex_cum_dialogue") from _call_expression_2129
    $ renpy.end_replay()
    $ game.timer.tick()
