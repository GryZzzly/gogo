...
label ross_office_dialogue:
    ...
    if game.timer.is_night():
        call expression game.dialog_select("bissette_office_night_lock") from _call_expression_1910
        $ player.go_to(L_school_floor3)

    if ross_office_first_visit and not game.timer.is_dark():
        call expression game.dialog_select("ross_office_first_visit") from _call_expression_1911
