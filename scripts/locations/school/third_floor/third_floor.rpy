label third_floor_dialogue:
    ...
    call pa_announcement from _call_pa_announcement_1
    ...
    if M_okita.is_state(S_okita_get_ingredients) and not player.has_item("tissue") and not M_dewitt.is_state(S_dewitt_paint_trail):
        call expression game.dialog_select("third_floor_okita_get_ingredients") from _call_expression_352
    elif M_roxxy.is_state(S_roxxy_teachers_event) and not game.timer.is_dark():
        call expression game.dialog_select("third_floor_roxxy_intro") from _call_expression_353
