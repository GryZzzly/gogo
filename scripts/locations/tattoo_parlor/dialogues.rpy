...
label tattoo_pick_up_boxes:
    ...
    if M_ross.get("failed pick up boxes"):
        call expression game.dialog_select("tattoo_boxes_try_again") from _call_expression_784
    else:

        call expression game.dialog_select("tattoo_boxes_intro") from _call_expression_785

    if player.has_required_str(6):
        call expression game.dialog_select("tattoo_boxes_str_pass") from _call_expression_786
        $ player.get_item("paint")
    else:

        call expression game.dialog_select("tattoo_boxes_str_fail") from _call_expression_787
