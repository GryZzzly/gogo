label trailer_dialogue:
    ...
    if M_roxxy.is_state(S_roxxy_studying_at_roxxys) and game.timer.is_evening():
        call expression game.dialog_select("trailer_roxxy_studying_at_roxxys") from _call_expression_2239
        $ M_roxxy.trigger(T_roxxy_study_at_mcs)

    elif M_roxxy.is_state(S_roxxy_get_uniform_on_doggo) and player.has_item("roxxy_uniform"):
        call expression game.dialog_select("trailer_roxxy_get_uniform_on_doggo") from _call_expression_2240
...
label trailer_foreclosed_dialogue:
    if M_roxxy.is_state(S_roxxy_check_trailer):
        call expression game.dialog_select("trailer_roxxy_check_trailer") from _call_expression_2241
        $ M_roxxy.trigger(T_roxxy_checked_trailer)
    else:

        call expression game.dialog_select("trailer_roxxy_trailer_foreclosed") from _call_expression_2242
    $ player.go_to(L_trailer)
    $ game.main()
