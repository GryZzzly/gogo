label job_done_dialogue(earnings):
    ...
    if not garden_firsttime:
        scene black
        with Pause(0.5)
        call expression game.dialog_select("garden_firsttime_text") from _call_expression_1030
        pause
        scene black with dissolve
        with Pause(0.5)
    ...
    if earnings > 0:
        ...
        call expression game.dialog_select("garden_firsttime_fail") from _call_expression_1031
    $ game.timer.tick()
    if earnings < 0:
        $ earnings = 0
    ...
    with dissolve
    $ in_garden = True
    call expression game.dialog_select("garden_dialogue") from _call_expression_1032
